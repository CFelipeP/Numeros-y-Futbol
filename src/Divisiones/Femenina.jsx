  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="res-row" style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.7rem 0.8rem", borderRadius: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", transition: "all 0.2s ease", cursor: "default" }}>
        ...
        <span className="res-score" style={{ fontSize: "0.8rem", fontWeight: 800, color: "var(--color-white)", fontFamily: "var(--font-heading)", letterSpacing: "1px", flexShrink: 0, textShadow: "0 0 10px rgba(236,72,153,0.3)" }}>{m.goles_local ?? "-"} - {m.goles_visitante ?? "-"}</span>
  ...
  return (
    <div className="feat-card" style={{ position: "relative", borderRadius: 20, overflow: "hidden", background: "linear-gradient(160deg,#1a1f35 0%,#0d1117 40%,#111827 100%)", border: "1px solid rgba(255,255,255,0.06)", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <div className="feat-content" style={{ position: "relative", padding: "1.6rem 1.4rem 1.4rem" }}>
        ...
        {match.id && onVerMas && (
          <button onClick={() => onVerMas(match.id)} className="feat-btn" style={{ width: "100%", padding: "0.6rem 1rem", background: "linear-gradient(135deg, rgba(236,72,153,0.12), rgba(236,72,153,0.04))", border: "1px solid rgba(236,72,153,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "1.2px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
        )}
      </div>
    </div>
  );
...
const PublicPlayerRow = ({ j }) => {
  ...
  return (
    <div className="pv-player-row">
      ...
      <div className="pv-row-stats pv-dk">{isGK ? (<><b style={{ color: "#f59e0b" }}>{j.goles_recibidos || 0}</b><small>GR</small><b style={{ color: "#10b981" }}>{j.vaya_invicta || 0}</b><small>VI</small></>) : (<><b style={{ color: "#ef4444" }}>{j.goles || 0}</b><small>G</small><b style={{ color: "#3b82f6" }}>{j.asistencias || 0}</b><small>A</small></>)}</div>
      ...
      <div className="pv-hero-card">
        ...
        <div className="pv-hero-content">
          ...
          <div className="pv-hero-stats-row">
            {[{ v: jugadores.length, l: "Jugadores", c: "#f1f5f9" }, { v: jugadores.filter(j => j.es_titular == 1).length, l: "Titulares", c: "#f59e0b" }, { v: totalGoles, l: "Goles", c: "#ef4444" }, { v: totalAsistencias, l: "Asistencias", c: "#3b82f6" }, { v: avgAge, l: "Edad prom.", c: "#8b5cf6" }].map((s, i) => (
                ...
                {subs.map(s => {
                  ...
                  return (
                    <div key={s.id} className="pv-sub-card" style={{ borderLeftColor: pi.color }}>
                      ...
                      <div className="pv-sub-mini-stats pv-dk">{pi.cat === "portero" ? (<><span style={{ color: "#f59e0b" }}>{s.goles_recibidos || 0}<small>GR</small></span><span style={{ color: "#10b981" }}>{s.vaya_invicta || 0}<small>VI</small></span></>) : (<><span style={{ color: "#ef4444" }}>{s.goles || 0}<small>G</small></span><span style={{ color: "#3b82f6" }}>{s.asistencias || 0}<small>A</small></span></>)}</div>
                    </div>
                  );
                })}
              </div>
...
export default function Femenina() {
  ...
  const openMatchDetail = useCallback((id) => {
    if (id) navigate(`/partido/${id}/femenina`);
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([safeFetch(`${API_BASE}get_tabla_femenina.php`), safeFetch(`${API_BASE}get_featured_match_femenina.php?t=${Date.now()}`), safeFetch(`${API_BASE}get_teams_femenina.php`), safeFetch(`${API_BASE}get_sidebar_matches_femenina.php${jornadaFilter !== "all" ? `?jornada=${jornadaFilter}` : ""}`), safeFetch(`${API_BASE}get_matches_femenina.php`)]).then(r => {
  ...
  useEffect(() => {
    ...
    safeFetch(`${API_BASE}get_sidebar_matches_femenina.php?jornada=${jornadaFilter}`).then(sD => {
    ...
    <>
      ...
      <section className="table-section" style={{ paddingBottom: 0 }}>
        <div className="container" style={{ marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}><IconTrophy /><h2 className="pd-main-title" style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 800, textTransform: "uppercase", margin: 0, background: "linear-gradient(90deg, #fff, var(--color-accent))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Femenina</h2></div>
          <p className="pd-main-sub" style={{ color: "var(--color-text-muted)", fontSize: "1rem", margin: 0, paddingLeft: "1.8rem" }}>Clasificación general, equipos y partido destacado de la Liga Femenina</p>
        </div>
        <div className="container" style={{ marginBottom: "2rem" }}>
          <div className="pd-tabs" style={{ display: "flex", gap: "0.5rem", background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 4, border: "1px solid rgba(255,255,255,0.05)" }}>
            {[{ key: "clasificacion", label: "Clasificación", icon: "📊" }, { key: "equipos", label: "Equipos", icon: "🛡️" }].map(tab => (
              <button key={tab.key} onClick={() => changeTab(tab.key)} className={`pd-tab${activeTab === tab.key ? " pd-tab-active" : ""}`} style={{ flex: 1, padding: "0.75rem 1rem", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "0.85rem", letterSpacing: "0.5px", textTransform: "uppercase", background: activeTab === tab.key ? "linear-gradient(135deg, rgba(236,72,153,0.2), rgba(236,72,153,0.08))" : "transparent", color: activeTab === tab.key ? "var(--color-accent)" : "var(--color-text-muted)", boxShadow: activeTab === tab.key ? "0 0 15px rgba(236,72,153,0.15)" : "none", transition: "all 0.3s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span>{tab.icon}</span>{tab.label}</button>
            ))}
          </div>
        </div>
              ...
              <div className="pd-teams-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.5rem" }}>
                {equipos.map(eq => { const stats = getTeamStats(eq.id); const pos = stats ? tabla.indexOf(stats) : -1; const badge = pos >= 0 ? getPosBadge(pos) : null; return (
                  <div key={eq.id} className="glass-card pd-team-card" style={{ padding: 0, overflow: "hidden", transition: "all 0.3s ease", borderLeft: badge ? `3px solid ${badge.color}` : "3px solid transparent" }}
                    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = "0 15px 40px rgba(0,0,0,0.4), 0 0 20px rgba(236,72,153,0.1)"; e.currentTarget.style.borderColor = "rgba(236,72,153,0.3)"; if (badge) e.currentTarget.style.borderLeftColor = badge.color; }}
                    ...
                    <div style={{ padding: "1.5rem" }}>
                      ...
                      {stats && (<><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.5rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem" }}>{[{ l: "PJ", v: stats.partidos_jugados, c: "var(--color-text-main)" }, { l: "G", v: stats.ganados, c: "#10b981" }, { l: "E", v: stats.empatados, c: "#f59e0b" }, { l: "P", v: stats.perdidos, c: "#ef4444" }].map((s, i) => (<div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: "1.1rem", fontWeight: 800, color: s.c, fontFamily: "var(--font-heading)", lineHeight: 1 }}>{s.v}</div><div style={{ fontSize: "0.65rem", color: "var(--color-text-muted)", marginTop: "0.2rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.l}</div></div>))}</div><div style={{ marginTop: "1rem", paddingTop: "0.8rem", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "0.78rem", color: "var(--color-text-muted)", fontWeight: 600 }}>Puntos</span><div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}><div style={{ width: 80, height: 4, borderRadius: 2, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 2, background: "linear-gradient(90deg, var(--color-accent), #db2777)", width: `${Math.min((stats.puntos / (tabla[0]?.puntos || 1)) * 100, 100)}%`, transition: "width 0.5s ease" }} /></div><span style={{ fontSize: "1.2rem", fontWeight: 900, color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 10px rgba(236,72,153,0.4)" }}>{stats.puntos}</span></div></div></>)}
                      <button onClick={() => openTeam(eq.id)} style={{ width: "100%", marginTop: "1.2rem", padding: "0.7rem", background: "linear-gradient(135deg, rgba(236,72,153,0.15), rgba(236,72,153,0.05))", border: "1px solid rgba(236,72,153,0.2)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "1px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }} onMouseEnter={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(236,72,153,0.3), rgba(236,72,153,0.1))"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(236,72,153,0.15), rgba(236,72,153,0.05))"; e.currentTarget.style.transform = "translateY(0)"; }}>Ver Plantilla Completa</button>
                    </div>
                  </div>
                ); })}
              </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════════════════════ */}
      <style>{`
.table-section{--color-accent:#ec4899!important;--color-accent-rgb:236,72,153!important}
