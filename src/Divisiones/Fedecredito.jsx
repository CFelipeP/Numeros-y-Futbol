  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="res-row" style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.7rem 0.8rem", borderRadius: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", transition: "all 0.2s ease", cursor: "default" }}>
        ...
        <span className="res-score" style={{ fontSize: "0.8rem", fontWeight: 800, color: "var(--color-white)", fontFamily: "var(--font-heading)", letterSpacing: "1px", flexShrink: 0, textShadow: "0 0 10px rgba(16,185,129,0.3)" }}>{m.goles_local ?? "-"} - {m.goles_visitante ?? "-"}</span>
...
const FeaturedMatchCard = ({ match, onVerMas }) => {
  ...
  const sc = isL ? "#10b981" : isF ? "#10b981" : "#f59e0b", sb = isL ? "rgba(239,68,68,0.15)" : isF ? "rgba(16,185,129,0.12)" : "rgba(245,158,11,0.12)";
  return (
      ...
      <div className="feat-content" style={{ position: "relative", padding: "1.6rem 1.4rem 1.4rem" }}>
        ...
        <div className="feat-matchup" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.6rem" }}>
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}>
            ...
            {hw && <span className="feat-winner-tag" style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#10b981", background: "rgba(16,185,129,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(16,185,129,0.15)" }}>Ganador</span>}
          </div>
          ...
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}>
            ...
            {aw && <span className="feat-winner-tag" style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#10b981", background: "rgba(16,185,129,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(16,185,129,0.15)" }}>Ganador</span>}
          </div>
        </div>
        ...
        {match.id && onVerMas && (
          <button onClick={() => onVerMas(match.id)} className="feat-btn" style={{ width: "100%", padding: "0.6rem 1rem", background: "linear-gradient(135deg, rgba(16,185,129,0.12), rgba(16,185,129,0.04))", border: "1px solid rgba(16,185,129,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "1.2px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
        )}
      </div>
    </div>
  );
...
const posiciones = [
  ...
  { value: "extremo_derecho", label: "Extremo Derecho", cat: "medio", color: "#6ee7b7", abbr: "ED" }, { value: "centrodelantero", label: "Centrodelantero", cat: "delantero", color: "#10b981", abbr: "DC" },
...
const catCfg = {
  ...
  delantero: { color: "#10b981", border: "rgba(239,68,68,0.2)", label: "DELANTEROS", grad: "linear-gradient(135deg,rgba(239,68,68,0.08),rgba(239,68,68,0.02))", icon: "🎯" },
};
const posCompat = {
  portero: ["portero"],
...
const PublicPlayerRow = ({ j }) => {
  ...
  return (
    <div className="pv-player-row">
      ...
      <div className="pv-row-stats pv-dk">{isGK ? (<><b style={{ color: "#f59e0b" }}>{j.goles_recibidos || 0}</b><small>GR</small><b style={{ color: "#10b981" }}>{j.vaya_invicta || 0}</b><small>VI</small></>) : (<><b style={{ color: "#10b981" }}>{j.goles || 0}</b><small>G</small><b style={{ color: "#3b82f6" }}>{j.asistencias || 0}</b><small>A</small></>)}</div>
      ...
      <div className="pv-hero-card">
        ...
        <div className="pv-hero-content">
          ...
          <div className="pv-hero-stats-row">
            {[{ v: jugadores.length, l: "Jugadores", c: "#f1f5f9" }, { v: jugadores.filter(j => j.es_titular == 1).length, l: "Titulares", c: "#f59e0b" }, { v: totalGoles, l: "Goles", c: "#10b981" }, { v: totalAsistencias, l: "Asistencias", c: "#3b82f6" }, { v: avgAge, l: "Edad prom.", c: "#8b5cf6" }].map((s, i) => (
                ...
                {subs.map(s => {
                  ...
                  return (
                    <div key={s.id} className="pv-sub-card" style={{ borderLeftColor: pi.color }}>
                      ...
                      <div className="pv-sub-mini-stats pv-dk">{pi.cat === "portero" ? (<><span style={{ color: "#f59e0b" }}>{s.goles_recibidos || 0}<small>GR</small></span><span style={{ color: "#10b981" }}>{s.vaya_invicta || 0}<small>VI</small></span></>) : (<><span style={{ color: "#10b981" }}>{s.goles || 0}<small>G</small></span><span style={{ color: "#3b82f6" }}>{s.asistencias || 0}<small>A</small></span></>)}</div>
                    </div>
                  );
                })}
              </div>
...
export default function Fedecredito() {
  ...
  const openMatchDetail = useCallback((id) => {
    if (id) navigate(`/partido/${id}/reservas`);
  }, [navigate]);
  ...
  const loadTeamDetail = async (id) => {
    ...
    try { const data = await safeFetch(`${API_BASE}get_equipo_detalle_reservas.php?id=${id}`); setExpandedTeam(data); }
  ...
  useEffect(() => {
    ...
    Promise.allSettled([safeFetch(`${API_BASE}get_tabla_reservas.php`), safeFetch(`${API_BASE}get_featured_match.php?division=reservas&t=${Date.now()}`), safeFetch(`${API_BASE}get_teams_reservas.php`), safeFetch(`${API_BASE}get_sidebar_matches_reservas.php${jornadaFilter !== "all" ? `?jornada=${jornadaFilter}` : ""}`), safeFetch(`${API_BASE}get_matches_reservas.php`)]).then(r => {
      ...
      setMatch(feat);
      if (!feat || !feat.home_name) { safeFetch(`${API_BASE}get_matches_reservas.php`).then(am => { const a = Array.isArray(am) ? am : []; const f = a.find(m => m.featured == 1 || m.destacado == 1); if (f) setMatch(normalizeMatch(f, tm)); }).catch(() => {}); }
    }).catch(err => { setError(err.message); }).finally(() => setLoading(false));
  }, [jornadaFilter]);

  useEffect(() => {
    if (jornadaFilter === "all") return;
    safeFetch(`${API_BASE}get_sidebar_matches_reservas.php?jornada=${jornadaFilter}`).then(sD => {
  ...
  if (loading) return (<><Header /><section className="table-section" style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ textAlign: "center" }}><div style={{ width: 48, height: 48, borderRadius: "50%", border: "3px solid rgba(16,185,129,0.2)", borderTopColor: "#10b981", animation: "spin 1s linear infinite", margin: "0 auto 1rem" }} /><p style={{ color: "var(--color-text-muted)", fontFamily: "var(--font-heading)", fontWeight: 600, letterSpacing: "1px" }}>CARGANDO DATOS...</p><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div></section></>);

  if (error) return (<><Header /><section className="table-section" style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div className="glass-card" style={{ maxWidth: 600, padding: "2.5rem", textAlign: "center", border: "1px solid rgba(16,185,129,0.3)" }}><div style={{ color: "#10b981", marginBottom: "1rem" }}><IconAlert /></div><h3 style={{ fontFamily: "var(--font-heading)", fontSize: "1.3rem", fontWeight: 800, color: "#10b981", marginBottom: "0.8rem" }}>Error al cargar los datos</h3><p style={{ color: "var(--color-text-muted)", fontSize: "0.9rem", lineHeight: 1.6, marginBottom: "1.5rem" }}>{error}</p><button onClick={() => window.location.reload()} style={{ background: "linear-gradient(90deg, var(--color-accent), #059669)", color: "white", border: "none", padding: "0.8rem 2rem", borderRadius: 10, cursor: "pointer", fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "1px" }}>Reintentar</button></div></section></>);
    ...
    <>
      ...
      <section className="table-section" style={{ paddingBottom: 0 }}>
        <div className="container" style={{ marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}><IconTrophy /><h2 className="pd-main-title" style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 800, textTransform: "uppercase", margin: 0, background: "linear-gradient(90deg, #fff, var(--color-accent))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Liga Fedecredito</h2></div>
          <p className="pd-main-sub" style={{ color: "var(--color-text-muted)", fontSize: "1rem", margin: 0, paddingLeft: "1.8rem" }}>Liga de Reservas — Primera División de El Salvador</p>
        </div>
        <div className="container" style={{ marginBottom: "2rem" }}>
          <div className="pd-tabs" style={{ display: "flex", gap: "0.5rem", background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 4, border: "1px solid rgba(255,255,255,0.05)" }}>
            {[{ key: "clasificacion", label: "Clasificación", icon: "📊" }, { key: "equipos", label: "Equipos", icon: "🛡️" }].map(tab => (
              <button key={tab.key} onClick={() => changeTab(tab.key)} className={`pd-tab${activeTab === tab.key ? " pd-tab-active" : ""}`} style={{ flex: 1, padding: "0.75rem 1rem", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "0.85rem", letterSpacing: "0.5px", textTransform: "uppercase", background: activeTab === tab.key ? "linear-gradient(135deg, rgba(16,185,129,0.2), rgba(16,185,129,0.08))" : "transparent", color: activeTab === tab.key ? "var(--color-accent)" : "var(--color-text-muted)", boxShadow: activeTab === tab.key ? "0 0 15px rgba(16,185,129,0.15)" : "none", transition: "all 0.3s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span>{tab.icon}</span>{tab.label}</button>
            ))}
          </div>
        </div>
          ...
          <div className="dashboard-grid">
            <div className="pd-sidebar">
              ...
              {(sidebar.next.id || sidebar.next.partido_id) && (
                <button onClick={() => openMatchDetail(sidebar.next.id || sidebar.next.partido_id)} className="next-match-btn" style={{ width: "100%", marginTop: "1rem", padding: "0.55rem 1rem", background: "linear-gradient(135deg, rgba(16,185,129,0.12), rgba(16,185,129,0.04))", border: "1px solid rgba(16,185,129,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "1px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.45rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
              )}
              ...
              {/* ÚLTIMOS RESULTADOS */}
              <div className="glass-card" style={{ padding: "1.8rem" }}><div className="section-subtitle" style={{ marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem", justifyContent: "center" }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#10b981", boxShadow: "0 0 8px #10b981", display: "inline-block" }} />Últimos Resultados de la jornada</div>
                {jornadas.length > 0 && (
              ...
              {/* LEYENDA */}
              <div className="glass-card" style={{ padding: "1.5rem" }}><div className="section-subtitle" style={{ marginTop: 0, fontSize: "0.85rem" }}>Leyenda</div><div style={{ display: "flex", flexDirection: "column", gap: "0.7rem" }}>{[{ color: "#10b981", label: "Clasificación a Liga Concacaf" }, { color: "#f59e0b", label: "Playoffs / Repechaje" }, { color: "#10b981", label: "Descenso directo" }].map((item, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}><span style={{ width: 10, height: 10, borderRadius: 2, background: item.color, flexShrink: 0, boxShadow: `0 0 6px ${item.color}40` }} /><span style={{ fontSize: "0.82rem", color: "var(--color-text-muted)" }}>{item.label}</span></div>))}<div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}><span style={{ fontSize: "0.7rem", fontWeight: 800, color: "var(--color-text-muted)", width: 18, textAlign: "center", flexShrink: 0 }}>DG</span><span style={{ fontSize: "0.82rem", color: "var(--color-text-muted)" }}>Diferencia de goles</span></div></div></div>
            </div>
              ...
              <div className="pd-teams-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.5rem" }}>
                {equipos.map(eq => { const stats = getTeamStats(eq.id); const pos = stats ? tabla.indexOf(stats) : -1; const badge = pos >= 0 ? getPosBadge(pos) : null; return (
                  <div key={eq.id} className="glass-card" style={{ padding: 0, overflow: "hidden", transition: "all 0.3s ease", borderLeft: badge ? `3px solid ${badge.color}` : "3px solid transparent" }}
                    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = "0 15px 40px rgba(0,0,0,0.4), 0 0 20px rgba(16,185,129,0.1)"; e.currentTarget.style.borderColor = "rgba(16,185,129,0.3)"; if (badge) e.currentTarget.style.borderLeftColor = badge.color; }}
                    ...
                    <div style={{ padding: "1.5rem" }}>
                      ...
                      {stats && (<><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.5rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem" }}>{[{ l: "PJ", v: stats.partidos_jugados, c: "var(--color-text-main)" }, { l: "G", v: stats.ganados, c: "#10b981" }, { l: "E", v: stats.empatados, c: "#f59e0b" }, { l: "P", v: stats.perdidos, c: "#ef4444" }].map((s, i) => (<div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: "1.1rem", fontWeight: 800, color: s.c, fontFamily: "var(--font-heading)", lineHeight: 1 }}>{s.v}</div><div style={{ fontSize: "0.65rem", color: "var(--color-text-muted)", marginTop: "0.2rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.l}</div></div>))}</div><div style={{ marginTop: "1rem", paddingTop: "0.8rem", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "0.78rem", color: "var(--color-text-muted)", fontWeight: 600 }}>Puntos</span><div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}><div style={{ width: 80, height: 4, borderRadius: 2, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 2, background: "linear-gradient(90deg, var(--color-accent), #059669)", width: `${Math.min((stats.puntos / (tabla[0]?.puntos || 1)) * 100, 100)}%`, transition: "width 0.5s ease" }} /></div><span style={{ fontSize: "1.2rem", fontWeight: 900, color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 10px rgba(16,185,129,0.4)" }}>{stats.puntos}</span></div></div></>)}
                      <button onClick={() => openTeam(eq.id)} style={{ width: "100%", marginTop: "1.2rem", padding: "0.7rem", background: "linear-gradient(135deg, rgba(16,185,129,0.15), rgba(16,185,129,0.05))", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "1px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }} onMouseEnter={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(16,185,129,0.3), rgba(16,185,129,0.1))"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(16,185,129,0.15), rgba(16,185,129,0.05))"; e.currentTarget.style.transform = "translateY(0)"; }}>Ver Plantilla Completa</button>
                    </div>
                  </div>
                ); })}
              </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════════════════════ */}
      <style>{`
.table-section{--color-accent:#10b981!important;--color-accent-rgb:16,185,129!important}
