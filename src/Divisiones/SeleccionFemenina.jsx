...
const posiciones = [
  { value: "portero", label: "Portera", cat: "portero", color: "#f59e0b", abbr: "POR" },
  { value: "lateral_izquierdo", label: "Lateral Izquierda", cat: "defensa", color: "#60a5fa", abbr: "LI" },
  { value: "lateral_derecho", label: "Lateral Derecha", cat: "defensa", color: "#60a5fa", abbr: "LD" },
  { value: "central", label: "Central", cat: "defensa", color: "#3b82f6", abbr: "DFC" },
  { value: "medio_defensivo", label: "Medio Defensiva", cat: "medio", color: "#34d399", abbr: "MCD" },
  { value: "medio_central", label: "Medio Central", cat: "medio", color: "#10b981", abbr: "MC" },
  { value: "medio_ofensivo", label: "Medio Ofensiva", cat: "medio", color: "#059669", abbr: "MCO" },
  { value: "extremo_izquierdo", label: "Extremo Izquierda", cat: "medio", color: "#6ee7b7", abbr: "EI" },
  { value: "extremo_derecho", label: "Extremo Derecha", cat: "medio", color: "#6ee7b7", abbr: "ED" },
  { value: "centrodelantero", label: "Centrodelantera", cat: "delantero", color: "#ef4444", abbr: "DC" },
  { value: "segundo_delantero", label: "2da Delantera", cat: "delantero", color: "#f87171", abbr: "SD" },
];
...
const catCfg = {
  portero: { color: "#f59e0b", border: "rgba(245,158,11,0.2)", label: "PORTERAS", grad: "linear-gradient(135deg,rgba(245,158,11,0.08),rgba(245,158,11,0.02))", icon: "🧤" },
  ...
  delantero: { color: "#ef4444", border: "rgba(239,68,68,0.2)", label: "DELANTERAS", grad: "linear-gradient(135deg,rgba(239,68,68,0.08),rgba(239,68,68,0.02))", icon: "🎯" },
};

export default function SeleccionFemenina() {
  ...
  useEffect(() => {
    fetch(`${API}get_seleccion_detalle_femenina.php`)
      .then(r => r.json())
      .then(d => {
        setData(d);
        setLoading(false);
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .sel-hero { position:relative; background:linear-gradient(160deg,#1a0a1e 0%,#1d0b1a 40%,#2c041c 100%); border-bottom:2px solid rgba(236,72,153,0.2); overflow:hidden; }
        .sel-hero::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 80% 60% at 50% 0%, rgba(236,72,153,0.08) 0%, transparent 70%); pointer-events:none; }
        .sel-hero-stripe { height:3px; background:linear-gradient(90deg,transparent,#ec489988,#ec4899,#ec489988,transparent); }
        .sel-hero-inner { max-width:900px; margin:0 auto; padding:32px 20px 36px; position:relative; z-index:1; text-align:center; }
        .sel-escudo { width:100px; height:100px; margin:0 auto 16px; border-radius:50%; background:rgba(255,255,255,0.04); border:3px solid rgba(236,72,153,0.3); padding:12px; display:flex; align-items:center; justify-content:center; box-shadow:0 12px 30px rgba(0,0,0,0.6); }
        ...
        .sel-tab.active { background:linear-gradient(135deg,rgba(236,72,153,0.18),rgba(236,72,153,0.06)); color:#ec4899; box-shadow:0 0 15px rgba(236,72,153,0.12); }
        ...
        .sel-staff-avatar { width:64px; height:64px; border-radius:50%; margin:0 auto 10px; background:rgba(236,72,153,0.1); border:2px solid rgba(236,72,153,0.2); display:flex; align-items:center; justify-content:center; overflow:hidden; }
        .sel-staff-avatar img { width:100%; height:100%; object-fit:cover; }
        .sel-staff-name { font-size:14px; font-weight:700; color:#f1f5f9; }
        .sel-staff-rol { font-size:11px; color:#ec4899; font-weight:600; margin-top:2px; }
      ...
      <div className="sel-body">
        <div className="sel-hero">
          ...
          <div className="sel-hero-inner">
            ...
            <h1>Selección Femenina Salvadoreña</h1>
            <p>{jugadores.length} convocadas · {staff.length} cuerpo técnico · {partidos.filter(p => (p.estado || "").toLowerCase() === "finalizado").length} partidos jugados</p>
          </div>
        </div>
        ...
        <div className="sel-content">
          <div className="sel-tabs">
            {[
              ...
              { key: "jugadores", label: "Jugadoras", icon: "👥" },
            ...
            <div className="sel-card">
              ...
              {partidos.length === 0 ? (
                ...
                partidos.map(p => {
                  ...
                  return (
                    <div key={p.id} className="sel-match-row" onClick={() => navigate(`/partido/${p.id}/seleccion-femenina`)}>
                      <span style={{ fontSize: 20, fontWeight: 900, color: "#ec4899" }}>🇸🇻</span>
          ...
          {activeTab === "jugadores" && (
            <div className="sel-card">
              <div className="sel-card-title">👥 Jugadoras convocadas ({jugadores.length})</div>
                  ...
                  <div key={cat} className="sel-player-group">
                    ...
                    {items.map(j => (
                      <div key={j.id} className="sel-player-row">
                        ...
                        <span className="sel-player-club">{j.club_origen || ""}</span>
