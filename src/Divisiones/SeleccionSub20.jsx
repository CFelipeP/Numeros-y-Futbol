...
export default function Seleccion() {
  ...
  useEffect(() => {
    fetch(`${API}get_seleccion_detalle_sub20.php`)
      .then(r => r.json())
      .then(d => {
        setData(d);
        setLoading(false);
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .sel-hero { position:relative; background:linear-gradient(160deg,#0a1a2e 0%,#0d1b2a 40%,#041c2c 100%); border-bottom:2px solid rgba(249,115,22,0.2); overflow:hidden; }
        .sel-hero::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 80% 60% at 50% 0%, rgba(249,115,22,0.08) 0%, transparent 70%); pointer-events:none; }
        .sel-hero-stripe { height:3px; background:linear-gradient(90deg,transparent,#f9731688,#f97316,#f9731688,transparent); }
        .sel-hero-inner { max-width:900px; margin:0 auto; padding:32px 20px 36px; position:relative; z-index:1; text-align:center; }
        .sel-escudo { width:100px; height:100px; margin:0 auto 16px; border-radius:50%; background:rgba(255,255,255,0.04); border:3px solid rgba(249,115,22,0.3); padding:12px; display:flex; align-items:center; justify-content:center; box-shadow:0 12px 30px rgba(0,0,0,0.6); }
        ...
        .sel-tab.active { background:linear-gradient(135deg,rgba(249,115,22,0.18),rgba(249,115,22,0.06)); color:#f97316; box-shadow:0 0 15px rgba(249,115,22,0.12); }
        ...
        .sel-staff-avatar { width:64px; height:64px; border-radius:50%; margin:0 auto 10px; background:rgba(249,115,22,0.1); border:2px solid rgba(249,115,22,0.2); display:flex; align-items:center; justify-content:center; overflow:hidden; }
        .sel-staff-avatar img { width:100%; height:100%; object-fit:cover; }
        .sel-staff-name { font-size:14px; font-weight:700; color:#f1f5f9; }
        .sel-staff-rol { font-size:11px; color:#f97316; font-weight:600; margin-top:2px; }
      ...
      <div className="sel-body">
        <div className="sel-hero">
          ...
          <div className="sel-hero-inner">
            ...
            <h1>Selección Sub-20 Salvadoreña</h1>
            ...
            <div className="sel-card">
              ...
              {partidos.length === 0 ? (
                ...
                partidos.map(p => {
                  ...
                  return (
                    <div key={p.id} className="sel-match-row" onClick={() => navigate(`/partido/${p.id}/seleccion-sub20`)}>
                      <span style={{ fontSize: 20, fontWeight: 900, color: "#f97316" }}>🇸🇻</span>
