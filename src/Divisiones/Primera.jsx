  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="res-row" style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.7rem 0.8rem", borderRadius: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", transition: "all 0.2s ease", cursor: "default" }}>
        ...
        <span className="res-name nm-name" style={{ fontSize: "0.75rem", fontWeight: hw ? 800 : 600, color: hw ? "var(--color-white)" : "var(--color-text-muted)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.home_name}</span>
        <span className="res-score" style={{ fontSize: "0.8rem", fontWeight: 800, color: "var(--color-white)", fontFamily: "var(--font-heading)", letterSpacing: "1px", flexShrink: 0, textShadow: "0 0 10px rgba(255,0,77,0.3)" }}>{m.goles_local ?? "-"} - {m.goles_visitante ?? "-"}</span>
        <span className="res-name res-name-away nm-name" style={{ fontSize: "0.75rem", fontWeight: aw ? 800 : 600, color: aw ? "var(--color-white)" : "var(--color-text-muted)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }}>{m.away_name}</span>
  ...
  return (
    <div className="feat-card" style={{ position: "relative", borderRadius: 20, overflow: "hidden", background: "linear-gradient(160deg,#1a1f35 0%,#0d1117 40%,#111827 100%)", border: "1px solid rgba(255,255,255,0.06)", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <div className="feat-content" style={{ position: "relative", padding: "1.6rem 1.4rem 1.4rem" }}>
        ...
        {match.id && onVerMas && (
          <button onClick={() => onVerMas(match.id)} className="feat-btn" style={{ width: "100%", padding: "0.6rem 1rem", background: "linear-gradient(135deg, rgba(255,0,77,0.12), rgba(255,0,77,0.04))", border: "1px solid rgba(255,0,77,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "1.2px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
        )}
      </div>
    </div>
  );
...
export default function PrimeraDivision() {
  ...
  const [sidebar, setSidebar] = useState({ next: null, nextMatches: [], recent: [] });
  ...
  const loadTeamDetail = async (id) => {
    ...
    try { const data = await safeFetch(`${API_BASE}get_equipo_detalle.php?id=${id}`); setExpandedTeam(data); }
  ...
  useEffect(() => {
    ...
    Promise.allSettled([safeFetch(`${API_BASE}get_tabla.php`), safeFetch(`${API_BASE}get_featured_match.php?t=${Date.now()}`), safeFetch(`${API_BASE}get_teams.php`), safeFetch(`${API_BASE}get_sidebar_matches.php${jornadaFilter !== "all" ? `?jornada=${jornadaFilter}` : ""}`), safeFetch(`${API_BASE}get_matches.php`)]).then(r => {
      ...
      setTabla(tA); setEquipos(eA); setSidebar(sD && typeof sD === "object" ? { next: sD.next, nextMatches: sD.nextMatches || [], recent: sD.recent || [] } : { next: null, nextMatches: [], recent: [] });
    ...
    <>
      ...
      <section className="table-section" style={{ paddingBottom: 0 }}>
        <div className="container" style={{ marginBottom: "1.5rem" }}>
          ...
          <p className="pd-main-sub" style={{ color: "var(--color-text-muted)", fontSize: "1rem", margin: 0, paddingLeft: "1.8rem" }}>Clasificación general, equipos y partido destacado de la Liga Mayor</p>
        </div>
        ...
        {activeTab === "clasificacion" && !expandedTeam && (
          <div className="dashboard-grid">
            <div className="pd-sidebar">
              ...
              {/* PRÓXIMOS PARTIDOS */}
              <div className="glass-card" style={{ padding: "1.8rem" }}><div className="section-subtitle" style={{ marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#f59e0b", boxShadow: "0 0 8px #f59e0b", display: "inline-block" }} />Próximos Partidos</div>{sidebar.nextMatches && sidebar.nextMatches.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.3rem" }}><span style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1px", color: "#f59e0b", background: "rgba(245,158,11,0.12)", padding: "0.2rem 0.6rem", borderRadius: 6 }}>Jornada {sidebar.nextMatches[0].jornada || sidebar.nextJornada || "\u2014"}</span><span style={{ fontSize: "0.6rem", color: "var(--color-text-muted)" }}>{sidebar.nextMatches.length} partido{sidebar.nextMatches.length > 1 ? "s" : ""}</span></div>{sidebar.nextMatches.map(m => (<div key={m.id || m.partido_id} onClick={() => openMatchDetail(m.id || m.partido_id)} style={{ background: "linear-gradient(135deg, rgba(245,158,11,0.04) 0%, rgba(30,41,59,0.3) 100%)", border: "1px solid rgba(245,158,11,0.10)", borderRadius: 12, padding: "0.8rem 0.9rem", cursor: "pointer", transition: "all 0.2s ease" }} className="nm-row" onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(245,158,11,0.3)"; e.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(245,158,11,0.10)"; e.currentTarget.style.transform = "translateY(0)"; }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.5rem" }}><div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flex: 1, minWidth: 0 }}><div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.06)", padding: 3, display: "flex", alignItems: "center", justifyContent: "center" }} className="nm-logo"><img src={logoUrl(m.home_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div><span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--color-text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} className="nm-name">{m.home_name}</span></div><span style={{ fontSize: "0.6rem", fontWeight: 800, color: "var(--color-text-muted)", background: "rgba(255,255,255,0.04)", padding: "0.2rem 0.4rem", borderRadius: 6, letterSpacing: "1px", flexShrink: 0 }} className="nm-vs">VS</span><div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flex: 1, minWidth: 0, justifyContent: "flex-end" }}><span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--color-text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }} className="nm-name">{m.away_name}</span><div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.06)", padding: 3, display: "flex", alignItems: "center", justifyContent: "center" }} className="nm-logo"><img src={logoUrl(m.away_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div></div></div>{m.fecha && <div style={{ display: "flex", alignItems: "center", gap: "0.3rem", marginTop: "0.4rem", fontSize: "0.62rem", color: "rgba(255,255,255,0.3)" }} className="nm-date"><IconCalendar /> {formatShortDate(m.fecha)}</div>}</div>))}</div>) : (<div style={{ textAlign: "center", padding: "1.5rem 1rem", color: "var(--color-text-muted)" }}><div style={{ fontSize: "1.3rem", marginBottom: "0.4rem", opacity: 0.3 }}>\xf0\x9f\x93\x85</div><p style={{ fontSize: "0.85rem", margin: 0, fontWeight: 600 }}>No hay partidos pendientes</p></div>)}</div>
              {/* ÚLTIMOS RESULTADOS */}
