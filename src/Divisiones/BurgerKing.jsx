...
const getPosBadge = (i) => {
  if (i === 0) return { bg: "rgba(245,158,11,0.15)", color: "#f59e0b" };
...
const ResultRow = ({ m, onVerMas }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="res-row" style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.7rem 0.8rem", borderRadius: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", transition: "all 0.2s ease", cursor: "default" }}>
        ...
        <span className="res-name" style={{ fontSize: "0.75rem", fontWeight: hw ? 800 : 600, color: hw ? "var(--color-white)" : "var(--color-text-muted)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{m.home_name}</span>
        <span className="res-score" style={{ fontSize: "0.8rem", fontWeight: 800, color: "var(--color-white)", fontFamily: "var(--font-heading)", letterSpacing: "1px", flexShrink: 0, textShadow: "0 0 10px rgba(245,158,11,0.3)" }}>{m.goles_local ?? "-"} - {m.goles_visitante ?? "-"}</span>
        <span className="res-name res-name-away" style={{ fontSize: "0.75rem", fontWeight: aw ? 800 : 600, color: aw ? "var(--color-white)" : "var(--color-text-muted)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }}>{m.away_name}</span>
      ...
      {m.id && onVerMas && (
        <button onClick={() => onVerMas(m.id)} className="res-more" style={{ alignSelf: "flex-end", display: "flex", alignItems: "center", gap: "0.35rem", background: "none", border: "none", color: "var(--color-accent)", fontSize: "0.68rem", fontWeight: 700, cursor: "pointer", padding: "0.25rem 0.6rem", borderRadius: 6, letterSpacing: "0.5px", textTransform: "uppercase", transition: "all 0.2s ease", marginTop: "0.15rem", marginRight: "0.2rem" }}>
          <span className="res-more-text">Ver mas</span> <IconArrowRight />
        </button>
      )}
    </div>
  );
...
const FeaturedMatchCard = ({ match, onVerMas }) => {
  ...
  const sc = isL ? "#f59e0b" : isF ? "#f59e0b" : "#f59e0b", sb = isL ? "rgba(239,68,68,0.15)" : isF ? "rgba(245,158,11,0.12)" : "rgba(245,158,11,0.12)";
  return (
    <div className="feat-card" style={{ position: "relative", borderRadius: 20, overflow: "hidden", background: "linear-gradient(160deg,#1a1f35 0%,#0d1117 40%,#111827 100%)", border: "1px solid rgba(255,255,255,0.06)", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <div className="feat-content" style={{ position: "relative", padding: "1.6rem 1.4rem 1.4rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.8rem" }}>
          ...
          <span className="feat-status" style={{ fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px", color: sc, background: sb, padding: "4px 12px", borderRadius: 20, border: `1px solid ${sc}25`, animation: isL ? "fp 2s ease-in-out infinite" : "none" }}>{st.text}</span>
        </div>
        <div className="feat-matchup" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.6rem" }}>
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}>
            <div className="feat-logo" style={{ width: 62, height: 62, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "2px solid rgba(255,255,255,0.06)", padding: 8, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.3)" }}><img src={logoUrl(match.home_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.3))" }} /></div>
            <span className="feat-team-name" style={{ fontSize: "0.72rem", fontWeight: hw ? 800 : 600, color: hw ? "#fff" : "rgba(255,255,255,0.55)", textAlign: "center", lineHeight: 1.25, maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{match.home_name}</span>
            {hw && <span className="feat-winner-tag" style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#f59e0b", background: "rgba(245,158,11,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(245,158,11,0.15)" }}>Ganador</span>}
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "0 0.8rem", flexShrink: 0 }}>
            <div className="feat-score-box" style={{ display: "flex", alignItems: "center", background: "rgba(0,0,0,0.35)", borderRadius: 14, padding: "6px 4px", border: "1px solid rgba(255,255,255,0.04)", boxShadow: "0 4px 20px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.03)" }}>
              <span className="feat-score-num" style={{ fontSize: "1.8rem", fontWeight: 900, fontFamily: "var(--font-heading)", color: hw ? "#fff" : "rgba(255,255,255,0.6)", width: 48, textAlign: "center", lineHeight: 1 }}>{match.goles_local ?? "-"}</span>
              <span style={{ fontSize: "1rem", color: "rgba(255,255,255,0.2)", margin: "0 2px" }}>:</span>
              <span className="feat-score-num" style={{ fontSize: "1.8rem", fontWeight: 900, fontFamily: "var(--font-heading)", color: aw ? "#fff" : "rgba(255,255,255,0.6)", width: 48, textAlign: "center", lineHeight: 1 }}>{match.goles_visitante ?? "-"}</span>
            </div>
          ...
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}>
            <div className="feat-logo" style={{ width: 62, height: 62, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "2px solid rgba(255,255,255,0.06)", padding: 8, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.3)" }}><img src={logoUrl(match.away_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.3))" }} /></div>
            <span className="feat-team-name" style={{ fontSize: "0.72rem", fontWeight: aw ? 800 : 600, color: aw ? "#fff" : "rgba(255,255,255,0.55)", textAlign: "center", lineHeight: 1.25, maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{match.away_name}</span>
            {aw && <span className="feat-winner-tag" style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#f59e0b", background: "rgba(245,158,11,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(245,158,11,0.15)" }}>Ganador</span>}
          </div>
        </div>
        ...
        {match.fecha && <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 6, fontSize: "0.7rem", color: "rgba(255,255,255,0.3)", marginBottom: "0.8rem" }}><IconCalendar /><span>{formatShortDate(match.fecha)}</span></div>}
        {match.id && onVerMas && (
          <button onClick={() => onVerMas(match.id)} className="feat-btn" style={{ width: "100%", padding: "0.6rem 1rem", background: "linear-gradient(135deg, rgba(245,158,11,0.12), rgba(245,158,11,0.04))", border: "1px solid rgba(245,158,11,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "1.2px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
        )}
      </div>
    </div>
  );
...
const posiciones = [
  ...
  { value: "medio_defensivo", label: "Medio Defensivo", cat: "medio", color: "#34d399", abbr: "MCD" }, { value: "medio_central", label: "Medio Central", cat: "medio", color: "#f59e0b", abbr: "MC" },
  { value: "medio_ofensivo", label: "Medio Ofensivo", cat: "medio", color: "#d97706", abbr: "MCO" }, { value: "extremo_izquierdo", label: "Extremo Izquierdo", cat: "medio", color: "#6ee7b7", abbr: "EI" },
  { value: "extremo_derecho", label: "Extremo Derecho", cat: "medio", color: "#6ee7b7", abbr: "ED" }, { value: "centrodelantero", label: "Centrodelantero", cat: "delantero", color: "#f59e0b", abbr: "DC" },
...
const catCfg = {
  ...
  medio: { color: "#f59e0b", border: "rgba(245,158,11,0.2)", label: "MEDIOS", grad: "linear-gradient(135deg,rgba(245,158,11,0.08),rgba(245,158,11,0.02))", icon: "⚽" },
  delantero: { color: "#f59e0b", border: "rgba(239,68,68,0.2)", label: "DELANTEROS", grad: "linear-gradient(135deg,rgba(239,68,68,0.08),rgba(239,68,68,0.02))", icon: "🎯" },
};
const posCompat = {
  portero: ["portero"],
...
const PublicPlayerRow = ({ j }) => {
  ...
  return (
    <div className="pv-player-row">
      ...
      <span className="pv-row-pos pv-dk" style={{ color: pi.color, borderColor: `${pi.color}30`, background: `${pi.color}0a` }} title={pi.label}>{pi.abbr}</span>
      <div className="pv-row-stats pv-dk">{isGK ? (<><b style={{ color: "#f59e0b" }}>{j.goles_recibidos || 0}</b><small>GR</small><b style={{ color: "#f59e0b" }}>{j.vaya_invicta || 0}</b><small>VI</small></>) : (<><b style={{ color: "#f59e0b" }}>{j.goles || 0}</b><small>G</small><b style={{ color: "#3b82f6" }}>{j.asistencias || 0}</b><small>A</small></>)}</div>
      <span className="pv-row-pj pv-dk">{j.pj || 0} <small>PJ</small></span>
    </div>
  );
};
...
const PublicPosGroup = ({ cat, jugadores }) => {
  ...
  return (
    <div className="pv-pos-group">
      <div className="pv-group-head" style={{ borderColor: cfg.border, background: cfg.grad }}>
        <span className="pv-group-icon">{cfg.icon}</span>
        <span className="pv-group-label" style={{ color: cfg.color }}>{cfg.label}</span>
        <span className="pv-group-count" style={{ color: cfg.color, background: `${cfg.color}12` }}>{jugadores.length}</span>
      </div>
      ...
      <div className="pv-hero-card">
        ...
        <div className="pv-hero-content">
          <div className="pv-hero-left">
            <div className="pv-hero-logo-ring"><img src={logoUrl(teamData?.equipo?.logo)} alt="" /></div>
            <div className="pv-hero-text">
              <h2>{teamData?.equipo?.nombre}</h2>
              <div className="pv-hero-meta">
                {[teamData?.equipo?.ciudad && <span key="c"><IconMapPin /> {teamData.equipo.ciudad}</span>, teamData?.equipo?.estadio && <span key="e"><IconStadium /> {teamData.equipo.estadio}</span>].filter(Boolean).map((el, i) => (
                  <React.Fragment key={i}>{i > 0 && <span className="pv-meta-sep">·</span>}{el}</React.Fragment>
                ))}
              </div>
            </div>
          </div>
          <div className="pv-hero-stats-row">
            {[{ v: jugadores.length, l: "Jugadores", c: "#f1f5f9" }, { v: jugadores.filter(j => j.es_titular == 1).length, l: "Titulares", c: "#f59e0b" }, { v: totalGoles, l: "Goles", c: "#f59e0b" }, { v: totalAsistencias, l: "Asistencias", c: "#3b82f6" }, { v: avgAge, l: "Edad prom.", c: "#8b5cf6" }].map((s, i) => (
              <React.Fragment key={i}>{i > 0 && <div className="pv-hero-stat-divider" />}<div className="pv-hero-stat"><strong style={{ color: s.c }}>{s.v}</strong><span>{s.l}</span></div></React.Fragment>
            ))}
          </div>
        </div>
      </div>
          ...
          <div className="pv-pitch-wrap">
            <div className="pv-pitch">
              ...
              {starters.map((s, idx) => {
                ...
                const isTit = s.es_titular == 1 || s.es_titular === true;
                return (
                  <div key={s.id || idx} className="pv-pp" style={{ left: `${s.px}%`, top: `${s.py}%`, animationDelay: `${idx * 0.06}s` }}>
                    <div className="pv-pp-dot" style={{ background: pi.color, boxShadow: `0 0 12px ${pi.color}50, inset 0 -2px 6px rgba(0,0,0,0.3)`, borderColor: isTit ? "rgba(255,255,255,0.45)" : "rgba(255,255,255,0.2)" }}>
                      {s.foto ? <img src={logoUrl(s.foto)} alt="" /> : <span>{s.numero_camiseta || "–"}</span>}
                    </div>
                    <span className="pv-pp-name">{s.nombre.split(" ").pop()}</span>
                    <span className="pv-pp-role">{pi.abbr}</span>
                  </div>
                );
              })}
                ...
                {subs.map(s => {
                  ...
                  return (
                    <div key={s.id} className="pv-sub-card" style={{ borderLeftColor: pi.color }}>
                      ...
                      <div className="pv-sub-mini-stats pv-dk">{pi.cat === "portero" ? (<><span style={{ color: "#f59e0b" }}>{s.goles_recibidos || 0}<small>GR</small></span><span style={{ color: "#f59e0b" }}>{s.vaya_invicta || 0}<small>VI</small></span></>) : (<><span style={{ color: "#f59e0b" }}>{s.goles || 0}<small>G</small></span><span style={{ color: "#3b82f6" }}>{s.asistencias || 0}<small>A</small></span></>)}</div>
                    </div>
                  );
                })}
              </div>
...
export default function BurgerKing() {
  ...
  const openMatchDetail = useCallback((id) => {
    if (id) navigate(`/partido/${id}/burgerking`);
  }, [navigate]);
  ...
  useEffect(() => {
    if (hash === "#equipos") { setActiveTab("equipos"); setExpandedTeam(null); }
      ...
      const parts = hash.replace("#equipo-", "").split("-");
      const id = parts[0], view = parts[1] || "plantilla";
      setExpandedViewTab(view);
      if (!expandedTeam || String(expandedTeam.equipo?.id) !== String(id)) loadTeamDetail(id);
    } else { setActiveTab("clasificacion"); setExpandedTeam(null); }
  }, [hash]);

  const loadTeamDetail = async (id) => {
    setExpandedTeam({ equipo: { id }, jugadores: [] });
    try { const data = await safeFetch(`${API_BASE}get_equipo_detalle_burgerking.php?id=${id}`); setExpandedTeam(data); }
  ...
  useEffect(() => {
    ...
    Promise.allSettled([safeFetch(`${API_BASE}get_tabla_burgerking.php`), safeFetch(`${API_BASE}get_featured_match.php?division=burgerking&t=${Date.now()}`), safeFetch(`${API_BASE}get_teams_burgerking.php`), safeFetch(`${API_BASE}get_sidebar_matches_burgerking.php${jornadaFilter !== "all" ? `?jornada=${jornadaFilter}` : ""}`), safeFetch(`${API_BASE}get_matches_burgerking.php`)]).then(r => {
      ...
      if (!feat || !feat.home_name) { if (sD?.recent?.length) { const f = sD.recent.find(m => m.featured == 1 || m.destacado == 1); if (f) feat = normalizeMatch(f, tm); } }
      if (!feat || !feat.home_name) { if (sD?.next && (sD.next.featured == 1 || sD.next.destacado == 1)) feat = normalizeMatch(sD.next, tm); }
      setMatch(feat);
      if (!feat || !feat.home_name) { safeFetch(`${API_BASE}get_matches_burgerking.php`).then(am => { const a = Array.isArray(am) ? am : []; const f = a.find(m => m.featured == 1 || m.destacado == 1); if (f) setMatch(normalizeMatch(f, tm)); }).catch(() => {}); }
    }).catch(err => { setError(err.message); }).finally(() => setLoading(false));
  }, [jornadaFilter]);

  useEffect(() => {
    if (jornadaFilter === "all") return;
    safeFetch(`${API_BASE}get_sidebar_matches_burgerking.php?jornada=${jornadaFilter}`).then(sD => {
  ...
  if (loading) return (<><Header /><section className="table-section" style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ textAlign: "center" }}><div style={{ width: 48, height: 48, borderRadius: "50%", border: "3px solid rgba(245,158,11,0.2)", borderTopColor: "#f59e0b", animation: "spin 1s linear infinite", margin: "0 auto 1rem" }} /><p style={{ color: "var(--color-text-muted)", fontFamily: "var(--font-heading)", fontWeight: 600, letterSpacing: "1px" }}>CARGANDO DATOS...</p><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div></section></>);

  if (error) return (<><Header /><section className="table-section" style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}><div className="glass-card" style={{ maxWidth: 600, padding: "2.5rem", textAlign: "center", border: "1px solid rgba(245,158,11,0.3)" }}><div style={{ color: "#f59e0b", marginBottom: "1rem" }}><IconAlert /></div><h3 style={{ fontFamily: "var(--font-heading)", fontSize: "1.3rem", fontWeight: 800, color: "#f59e0b", marginBottom: "0.8rem" }}>Error al cargar los datos</h3><p style={{ color: "var(--color-text-muted)", fontSize: "0.9rem", lineHeight: 1.6, marginBottom: "1.5rem" }}>{error}</p><button onClick={() => window.location.reload()} style={{ background: "linear-gradient(90deg, var(--color-accent), #d97706)", color: "white", border: "none", padding: "0.8rem 2rem", borderRadius: 10, cursor: "pointer", fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "0.85rem", textTransform: "uppercase", letterSpacing: "1px" }}>Reintentar</button></div></section></>);
    ...
    <>
      ...
      <section className="table-section" style={{ paddingBottom: 0 }}>
        <div className="container" style={{ marginBottom: "1.5rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.5rem" }}><IconTrophy /><h2 className="pd-main-title" style={{ fontFamily: "var(--font-heading)", fontSize: "2rem", fontWeight: 800, textTransform: "uppercase", margin: 0, background: "linear-gradient(90deg, #fff, var(--color-accent))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>Copa Burger King (U17)</h2></div>
          <p className="pd-main-sub" style={{ color: "var(--color-text-muted)", fontSize: "1rem", margin: 0, paddingLeft: "1.8rem" }}>Torneo Sub-17 — Copa Burger King</p>
        </div>
        <div className="container" style={{ marginBottom: "2rem" }}>
          <div className="pd-tabs" style={{ display: "flex", gap: "0.5rem", background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 4, border: "1px solid rgba(255,255,255,0.05)" }}>
            {[{ key: "clasificacion", label: "Clasificación", icon: "📊" }, { key: "equipos", label: "Equipos", icon: "🛡️" }].map(tab => (
              <button key={tab.key} onClick={() => changeTab(tab.key)} className={`pd-tab${activeTab === tab.key ? " pd-tab-active" : ""}`} style={{ flex: 1, padding: "0.75rem 1rem", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "0.85rem", letterSpacing: "0.5px", textTransform: "uppercase", background: activeTab === tab.key ? "linear-gradient(135deg, rgba(245,158,11,0.2), rgba(245,158,11,0.08))" : "transparent", color: activeTab === tab.key ? "var(--color-accent)" : "var(--color-text-muted)", boxShadow: activeTab === tab.key ? "0 0 15px rgba(245,158,11,0.15)" : "none", transition: "all 0.3s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}><span>{tab.icon}</span>{tab.label}</button>
            ))}
          </div>
        </div>
        ...
        {activeTab === "clasificacion" && !expandedTeam && (
          <div className="dashboard-grid">
            <div className="pd-sidebar">
              {/* PARTIDO DESTACADO */}
              <div className="pd-featured">{match && match.home_name ? <FeaturedMatchCard match={match} onVerMas={openMatchDetail} /> : <div className="glass-card" style={{ padding: "2.5rem 1.5rem", textAlign: "center" }}><div style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(255,255,255,0.03)", border: "2px dashed rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}><span style={{ fontSize: "1.5rem", opacity: 0.3 }}>⚽</span></div><p style={{ fontSize: "0.9rem", margin: "0 0 0.3rem", color: "var(--color-text-muted)", fontWeight: 600 }}>Sin partido destacado</p><p style={{ fontSize: "0.75rem", margin: 0, color: "rgba(255,255,255,0.25)" }}>Se mostrará cuando se configure desde el panel</p></div>}</div>

              {/* PRÓXIMO PARTIDO */}
              <div className="glass-card" style={{ padding: "1.8rem" }}><div className="section-subtitle" style={{ marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem" }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#f59e0b", boxShadow: "0 0 8px #f59e0b", display: "inline-block" }} />Próximo Partido</div>{sidebar.next ? (<div className="next-match-card" style={{ background: "linear-gradient(135deg, rgba(245,158,11,0.06) 0%, rgba(30,41,59,0.4) 100%)", border: "1px solid rgba(245,158,11,0.12)", borderRadius: 16, padding: "1.5rem 1.2rem" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.2rem" }}><span style={{ fontSize: "0.65rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1px", color: "#f59e0b", background: "rgba(245,158,11,0.12)", padding: "0.2rem 0.6rem", borderRadius: 6 }}>{getMatchStatus(sidebar.next.estado || sidebar.next.status).text}</span></div><div className="next-match-teams" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}><div className="next-match-team" style={{ display: "flex", alignItems: "center", gap: "0.7rem", flex: 1, minWidth: 0 }}><div style={{ width: 44, height: 44, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.06)", padding: 5, display: "flex", alignItems: "center", justifyContent: "center" }} className="nm-logo"><img src={logoUrl(sidebar.next.home_logo || sidebar.next.local_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div><span className="next-team-name" style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--color-text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sidebar.next.home_name || sidebar.next.local_nombre}</span></div><div style={{ flexShrink: 0 }}><span style={{ fontSize: "0.75rem", fontWeight: 800, color: "var(--color-text-muted)", background: "rgba(255,255,255,0.04)", padding: "0.35rem 0.7rem", borderRadius: 8, letterSpacing: "1px" }}>VS</span></div><div className="next-match-team next-match-team-r" style={{ display: "flex", alignItems: "center", gap: "0.7rem", flex: 1, minWidth: 0, justifyContent: "flex-end" }}><span className="next-team-name" style={{ fontSize: "0.82rem", fontWeight: 700, color: "var(--color-text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textAlign: "right" }}>{sidebar.next.away_name || sidebar.next.visitante_nombre}</span><div style={{ width: 44, height: 44, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.06)", padding: 5, display: "flex", alignItems: "center", justifyContent: "center" }} className="nm-logo"><img src={logoUrl(sidebar.next.away_logo || sidebar.next.visitante_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div></div></div>{sidebar.next.fecha && <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "0.8rem", marginTop: "1rem", display: "flex", alignItems: "center", gap: "0.4rem", fontSize: "0.78rem", color: "var(--color-text-muted)" }}><IconCalendar /> {sidebar.next.fecha}</div>}
              {(sidebar.next.id || sidebar.next.partido_id) && (
                <button onClick={() => openMatchDetail(sidebar.next.id || sidebar.next.partido_id)} className="next-match-btn" style={{ width: "100%", marginTop: "1rem", padding: "0.55rem 1rem", background: "linear-gradient(135deg, rgba(245,158,11,0.12), rgba(245,158,11,0.04))", border: "1px solid rgba(245,158,11,0.18)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.72rem", textTransform: "uppercase", letterSpacing: "1px", cursor: "pointer", transition: "all 0.25s ease", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.45rem" }}><span className="res-more-text">Ver mas</span> <IconArrowRight /></button>
              )}
              ...
              {/* ÚLTIMOS RESULTADOS */}
              <div className="glass-card" style={{ padding: "1.8rem" }}><div className="section-subtitle" style={{ marginTop: 0, display: "flex", alignItems: "center", gap: "0.5rem", justifyContent: "center" }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: "#f59e0b", boxShadow: "0 0 8px #f59e0b", display: "inline-block" }} />Últimos Resultados de la jornada</div>
                {jornadas.length > 0 && (
                ...
                {sidebar.recent && sidebar.recent.length > 0 ? (<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>{sidebar.recent.map(m => { const nm = normalizeMatch(m, teamMap); return (<div key={m.id}><ResultRow m={nm} onVerMas={openMatchDetail} />{m.fecha && <div style={{ display: "flex", alignItems: "center", gap: "0.3rem", paddingLeft: "0.8rem", paddingTop: "0.15rem", paddingBottom: "0.3rem", fontSize: "0.65rem", color: "rgba(255,255,255,0.25)" }}><IconClock /> {formatShortDate(m.fecha)}{m.jornada && <span style={{ marginLeft: "0.5rem", color: "#60a5fa", fontWeight: 700 }}>J{m.jornada}</span>}</div>}</div>); })}</div>) : (<div style={{ textAlign: "center", padding: "1.5rem 1rem", color: "var(--color-text-muted)" }}><div style={{ fontSize: "1.3rem", marginBottom: "0.4rem", opacity: 0.3 }}>📋</div><p style={{ fontSize: "0.85rem", margin: 0 }}>No hay resultados aún</p></div>)}</div>

              {/* LEYENDA */}
              <div className="glass-card" style={{ padding: "1.5rem" }}><div className="section-subtitle" style={{ marginTop: 0, fontSize: "0.85rem" }}>Leyenda</div><div style={{ display: "flex", flexDirection: "column", gap: "0.7rem" }}>{[{ color: "#1ff50b", label: "Clasificación a Liga Concacaf" }, { color: "#f59e0b", label: "Playoffs / Repechaje" }, { color: "#f50b0b", label: "Descenso directo" }].map((item, i) => (<div key={i} style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}><span style={{ width: 10, height: 10, borderRadius: 2, background: item.color, flexShrink: 0, boxShadow: `0 0 6px ${item.color}40` }} /><span style={{ fontSize: "0.82rem", color: "var(--color-text-muted)" }}>{item.label}</span></div>))}<div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}><span style={{ fontSize: "0.7rem", fontWeight: 800, color: "var(--color-text-muted)", width: 18, textAlign: "center", flexShrink: 0 }}>DG</span><span style={{ fontSize: "0.82rem", color: "var(--color-text-muted)" }}>Diferencia de goles</span></div></div></div>
            </div>

            {/* CLASIFICACIÓN */}
            <div className="pd-clasif-col glass-card" style={{ padding: "1.8rem" }}>
              ...
              <div className="pd-table-scroll"><table className="standings-table"><thead><tr><th className="pd-th-pos" style={{ width: 40, textAlign: "center" }}>#</th><th style={{ textAlign: "left", paddingLeft: 16 }}>Equipo</th><th>PJ</th><th>G</th><th>E</th><th>P</th><th className="pd-hide-m">GF</th><th className="pd-hide-m">GC</th><th>DG</th><th style={{ minWidth: 50 }}>PTS</th></tr></thead><tbody>{tabla.length === 0 && <tr><td colSpan={10} style={{ textAlign: "center", padding: "3rem 1rem", color: "var(--color-text-muted)" }}>No hay datos disponibles</td></tr>}{tabla.map((team, index) => { const badge = getPosBadge(index); const dg = getDG(team.goles_favor, team.goles_contra); const isBottom = index >= tabla.length - 1 && tabla.length > 4; return (<tr key={team.id} className="standings-row" style={{ borderLeft: badge ? `3px solid ${badge.color}` : isBottom ? "3px solid rgba(239,68,68,0.4)" : "3px solid transparent", transition: "all 0.2s ease" }}><td style={{ textAlign: "center" }}>{badge ? <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 24, height: 24, borderRadius: 6, background: badge.bg, color: badge.color, fontSize: "0.7rem", fontWeight: 800, fontFamily: "var(--font-heading)" }}>{index + 1}</span> : <span style={{ fontSize: "0.85rem", fontWeight: 600, color: isBottom ? "#f59e0b" : "var(--color-text-muted)" }}>{index + 1}</span>}</td><td className="team-cell" style={{ paddingLeft: 16 }}><img src={logoUrl(team.logo)} alt="" style={{ width: 28, height: 28, objectFit: "contain", background: "rgba(255,255,255,0.06)", borderRadius: "50%", padding: 3 }} /><span className="pd-team-name" style={{ fontWeight: 700, fontSize: "0.88rem", color: "var(--color-text-main)", whiteSpace: "nowrap" }}>{team.nombre}</span></td><td>{team.partidos_jugados}</td><td style={{ color: "#10b981", fontWeight: 600 }}>{team.ganados}</td><td style={{ color: "#f59e0b", fontWeight: 600 }}>{team.empatados}</td><td style={{ color: "#ef4444", fontWeight: 600 }}>{team.perdidos}</td><td className="pd-hide-m">{team.goles_favor}</td><td className="pd-hide-m">{team.goles_contra}</td><td style={{ fontWeight: 700, color: team.goles_favor - team.goles_contra > 0 ? "#f59e0b" : team.goles_favor - team.goles_contra < 0 ? "#ef4444" : "var(--color-text-muted)", fontSize: "0.85rem" }}>{dg}</td><td style={{ fontWeight: 800, fontSize: "1rem", color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 8px rgba(245,158,11,0.3)" }}>{team.puntos}</td></tr>); })}</tbody></table></div>
              ...
              <div className="pd-teams-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "1.5rem" }}>
                {equipos.map(eq => { const stats = getTeamStats(eq.id); const pos = stats ? tabla.indexOf(stats) : -1; const badge = pos >= 0 ? getPosBadge(pos) : null; return (
                  <div key={eq.id} className="glass-card" style={{ padding: 0, overflow: "hidden", transition: "all 0.3s ease", borderLeft: badge ? `3px solid ${badge.color}` : "3px solid transparent" }}
                    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = "0 15px 40px rgba(0,0,0,0.4), 0 0 20px rgba(245,158,11,0.1)"; e.currentTarget.style.borderColor = "rgba(245,158,11,0.3)"; if (badge) e.currentTarget.style.borderLeftColor = badge.color; }}
                    ...
                    <div style={{ padding: "1.5rem" }}>
                      ...
                      {stats && (<><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.5rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem" }}>{[{ l: "PJ", v: stats.partidos_jugados, c: "var(--color-text-main)" }, { l: "G", v: stats.ganados, c: "#10b981" }, { l: "E", v: stats.empatados, c: "#f59e0b" }, { l: "P", v: stats.perdidos, c: "#ef4444" }].map((s, i) => (<div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: "1.1rem", fontWeight: 800, color: s.c, fontFamily: "var(--font-heading)", lineHeight: 1 }}>{s.v}</div><div style={{ fontSize: "0.65rem", color: "var(--color-text-muted)", marginTop: "0.2rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.l}</div></div>))}</div><div style={{ marginTop: "1rem", paddingTop: "0.8rem", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "0.78rem", color: "var(--color-text-muted)", fontWeight: 600 }}>Puntos</span><div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}><div style={{ width: 80, height: 4, borderRadius: 2, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 2, background: "linear-gradient(90deg, var(--color-accent), #d97706)", width: `${Math.min((stats.puntos / (tabla[0]?.puntos || 1)) * 100, 100)}%`, transition: "width 0.5s ease" }} /></div><span style={{ fontSize: "1.2rem", fontWeight: 900, color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 10px rgba(245,158,11,0.4)" }}>{stats.puntos}</span></div></div></>)}
                      <button onClick={() => openTeam(eq.id)} style={{ width: "100%", marginTop: "1.2rem", padding: "0.7rem", background: "linear-gradient(135deg, rgba(245,158,11,0.15), rgba(245,158,11,0.05))", border: "1px solid rgba(245,158,11,0.2)", borderRadius: 10, color: "var(--color-accent)", fontWeight: 700, fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "1px", cursor: "pointer", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }} onMouseEnter={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(245,158,11,0.3), rgba(245,158,11,0.1))"; e.currentTarget.style.transform = "translateY(-2px)"; }} onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(245,158,11,0.15), rgba(245,158,11,0.05))"; e.currentTarget.style.transform = "translateY(0)"; }}>Ver Plantilla Completa</button>
                    </div>
                  </div>
                ); })}
              </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════════════════════ */}
      <style>{`
.table-section{--color-accent:#f59e0b!important;--color-accent-rgb:245,158,11!important}
...
`}</style>
      {/* ═══════════════════════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════════════════════ */}
      <style>{`
/* Clase auxiliar para ocultar en móvil */
@media(max-width:768px){
  .pd-hide-m{display:none!important}
  .pv-dk{display:none!important}
}

/* ── Layout: en móvil la tabla va arriba, sidebar abajo ── */
@media(max-width:768px){
  .dashboard-grid{
...
/* ── Título principal ── */
@media(max-width:768px){
  .pd-main-title{font-size:1.5rem!important}
  .pd-main-sub{font-size:.88rem!important;padding-left:0!important}
}

/* ── Tabs ── */
@media(max-width:768px){
  .pd-tabs{border-radius:10px!important}
  .pd-tab{padding:.6rem .7rem!important;font-size:.78rem!important;gap:.3rem!important}
}

/* ── Tabla: scroll horizontal con indicador ── */
@media(max-width:768px){
  .pd-table-scroll{
  ...
  @keyframes scrollHint{
    0%,100%{opacity:.15;transform:translateY(-50%) translateX(0)}
    50%{opacity:.5;transform:translateY(-50%) translateX(5px)}
  }
  .standings-table{min-width:380px!important}
  .standings-table th,
  .standings-table td{padding:8px 5px!important;font-size:.78rem!important}
  .pd-th-pos{width:32px!important}
  .pd-team-name{font-size:.8rem!important;max-width:100px!important;overflow:hidden!important;text-overflow:ellipsis!important;display:inline-block!important;vertical-align:middle}
  .team-cell img{width:24px!important;height:24px!important}
}

@media(max-width:480px){
  .standings-table{min-width:320px!important}
  .pd-team-name{font-size:.75rem!important;max-width:80px!important}
  .team-cell img{width:22px!important;height:22px!important;padding:2px!important}
  .standings-table th,
  .standings-table td{padding:7px 4px!important;font-size:.74rem!important}
}

/* ── Próximo partido: stack vertical ── */
@media(max-width:768px){
  .next-match-teams{
    flex-direction:column!important;
    gap:.8rem!important;
  ...
  .next-team-name{text-align:center!important}
  .next-match-team-r .next-team-name{text-align:center!important}
  .next-match-btn{min-height:44px}
}

/* ── Featured Match Card ── */
@media(max-width:768px){
  .feat-content{padding:1.2rem 1rem 1rem!important}
  .feat-content > div:first-child{margin-bottom:1.2rem!important}
  .feat-logo{width:48px!important;height:48px!important;padding:6px!important}
  .feat-team-name{font-size:.65rem!important;max-width:80px!important}
  .feat-winner-tag{font-size:.48rem!important;padding:1px 6px!important}
  .feat-score-num{font-size:1.4rem!important;width:36px!important}
  .feat-score-box{padding:5px 3px!important}
  .feat-matchup{gap:.4rem!important}
  .feat-btn{min-height:44px;font-size:.72rem!important}
}

@media(max-width:480px){
  .feat-content{padding:1rem .8rem!important}
  .feat-logo{width:42px!important;height:42px!important;padding:5px!important}
  .feat-team-name{font-size:.6rem!important;max-width:68px!important}
  .feat-score-num{font-size:1.2rem!important;width:30px!important}
  .feat-score-box{padding:4px 2px!important}
  .feat-winner-tag{display:none!important}
  .feat-btn{padding:.55rem .8rem!important;font-size:.68rem!important}
}

/* ── Result rows ── */
@media(max-width:768px){
  .res-row{padding:.55rem .6rem!important;gap:.4rem!important}
  .res-row > div:first-child,
  .res-row > div:last-child{width:26px!important;height:26px!important;padding:2px!important}
  .res-name{font-size:.7rem!important}
  .res-score{font-size:.75rem!important}
  .res-more{min-height:36px;font-size:.65rem!important}
}

@media(max-width:480px){
  .res-row > div:first-child,
  .res-row > div:last-child{width:22px!important;height:22px!important}
  .res-name{font-size:.65rem!important}
  .res-score{font-size:.7rem!important}
}
...
@media(max-width:768px){
  ...
  .pd-team-card{padding:0!important}
  .pd-team-btn{min-height:44px}
}

/* ── Sidebar cards padding ── */
@media(max-width:768px){
  .pd-sidebar .glass-card{padding:1.2rem!important}
}

/* ── Glass card padding general ── */
@media(max-width:480px){
  .pd-sidebar .glass-card{padding:1rem!important}
}

/* ═══════════════════════════════════════════════════════════
   TEAM VIEW — Responsive
   ═══════════════════════════════════════════════════════════ */

/* ── Back button ── */
@media(max-width:768px){
  .pv-back-btn{min-height:44px;font-size:.8rem!important}
}
...
@media(max-width:768px){
  ...
  .pv-tab-btn{min-height:42px;font-size:.8rem!important}
}
...
@media(max-width:360px){
  .pv-pitch-wrap{max-width:200px!important}
  .pv-pp-dot{width:18px!important;height:18px!important;font-size:.36rem!important;border-width:1px!important}
  .pv-pp-name{font-size:.32rem!important;max-width:32px!important}
}
...
@media(max-width:768px) and (orientation:landscape){
  ...
  .feat-matchup{gap:.5rem!important}
  .feat-logo{width:44px!important;height:44px!important}
}

/* ── Safe area (notch iPhones) ── */
@supports(padding:max(0px)){
  .pv-back-btn{
    padding-left:max(.55rem,env(safe-area-inset-left))!important;
    padding-right:max(.55rem,env(safe-area-inset-right))!important;
  }
  .feat-btn,
  .next-match-btn,
  .pd-team-btn{
...
@media(max-width:768px){
  ...
  .pd-table-scroll::-webkit-scrollbar{height:3px}
  .pd-table-scroll::-webkit-scrollbar-track{background:transparent}
  .pd-table-scroll::-webkit-scrollbar-thumb{background:rgba(255,255,255,.1);border-radius:2px}
}

/* ── iPhone SE / Extra small (≤375px) ── */
@media(max-width:375px){
  .pd-main-title{font-size:1rem!important}
  .pd-main-sub{font-size:.72rem!important}
  .pd-tabs{gap:1px!important;border-radius:8px!important}
  .pd-tab{font-size:.62rem!important;padding:.4rem .3rem!important}
  .standings-table{min-width:280px!important}
  .standings-table th,.standings-table td{padding:5px 3px!important;font-size:.62rem!important}
  .pd-team-name{font-size:.65rem!important;max-width:65px!important}
  .team-cell img{width:18px!important;height:18px!important}
  .pd-table-scroll{margin:0 -.6rem!important;padding:0 .6rem!important;width:calc(100% + 1.2rem)!important}
  .feat-content{padding:.8rem .6rem!important}
  .feat-logo{width:34px!important;height:34px!important}
  .feat-score-num{font-size:1.1rem!important}
  .feat-team-name{font-size:.55rem!important;max-width:55px!important}
  .res-row{padding:.4rem!important;font-size:.65rem!important}
  .next-match-teams{gap:.5rem!important}
  .pd-teams-grid{grid-template-columns:1fr!important;gap:.6rem!important}
  .pv-hero{padding:1.2rem .8rem!important}
  .pv-hero-left{gap:.8rem!important}
  .pv-hero-logo{width:52px!important;height:52px!important}
  .pv-hero-name{font-size:1rem!important}
  .pv-hero-stat-num{font-size:1.1rem!important}
  .pv-hero-stat-label{font-size:.55rem!important}
  .pv-pitch-wrap{max-width:210px!important}
  .pv-pp-dot{width:20px!important;height:20px!important;font-size:.38rem!important}
  .pv-pp-name{font-size:.34rem!important;max-width:36px!important}
}
      `}</style>
    </>
  );
