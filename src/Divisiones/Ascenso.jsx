  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="res-row" style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.7rem 0.8rem", borderRadius: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", transition: "all 0.2s ease", cursor: "default" }}
        ...
        <span className="res-score" style={{ fontSize: "0.8rem", fontWeight: 800, color: "var(--color-white)", fontFamily: "var(--font-heading)", letterSpacing: "1px", flexShrink: 0, textShadow: "0 0 10px rgba(34,197,94,0.3)" }}>{m.goles_local ?? "-"} - {m.goles_visitante ?? "-"}</span>
      ...
      {m.id && onVerMas && (
        <button onClick={() => onVerMas(m.id)} className="res-more" style={{
...
const TablaClasificacion = ({ datos, playoffTeamIds }) => (
  <div className="table-container asc-table-scroll-m">
    <table className="standings-table asc-table-m"><thead><tr><th className="asc-th-pos-m" style={{ width: 40, textAlign: "center" }}>#</th><th style={{ textAlign: "left", paddingLeft: 16 }}>Equipo</th><th>PJ</th><th>G</th><th>E</th><th>P</th><th className="asc-m-hide">GF</th><th className="asc-m-hide">GC</th><th>DG</th><th style={{ minWidth: 50 }}>PTS</th></tr></thead><tbody>
      {datos.length === 0 && <tr><td colSpan={10} style={{ textAlign: "center", padding: "2rem 1rem", color: "var(--color-text-muted)" }}>Sin equipos en esta división</td></tr>}
      {datos.map((team, index) => { const isPO = playoffTeamIds?.has(team.equipo_id); const dg = getDG(team.gf, team.gc); return (
        <tr key={team.id} style={{ borderLeft: isPO ? "3px solid #10b981" : "3px solid transparent", transition: "all 0.2s ease", background: isPO ? "rgba(16,185,129,0.04)" : "transparent" }}>
          ...
          <td className="team-cell" style={{ paddingLeft: 16 }}>{team.logo && <img src={logoUrl(team.logo)} alt="" style={{ width: 28, height: 28, objectFit: "contain", background: "rgba(255,255,255,0.06)", borderRadius: "50%", padding: 3 }} />}<span className="asc-team-name-m" style={{ fontWeight: 700, fontSize: "0.88rem", color: "var(--color-text-main)", whiteSpace: "nowrap" }}>{team.nombre}</span></td>
          <td>{team.pj}</td><td style={{ color: "#10b981", fontWeight: 600 }}>{team.pg}</td><td style={{ color: "#f59e0b", fontWeight: 600 }}>{team.pe}</td><td style={{ color: "#ef4444", fontWeight: 600 }}>{team.pp}</td><td className="asc-m-hide">{team.gf}</td><td className="asc-m-hide">{team.gc}</td><td style={{ fontWeight: 700, color: team.gf - team.gc > 0 ? "#10b981" : team.gf - team.gc < 0 ? "#ef4444" : "var(--color-text-muted)", fontSize: "0.85rem" }}>{dg}</td><td style={{ fontWeight: 800, fontSize: "1rem", color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 8px rgba(34,197,94,0.3)" }}>{team.pts}</td>
        </tr>);
      })}
    </tbody></table>
  </div>
  ...
  return (
    <div className="feat-card" style={{ position: "relative", borderRadius: 20, overflow: "hidden", background: "linear-gradient(160deg,#1a1f35 0%,#0d1117 40%,#111827 100%)", border: "1px solid rgba(255,255,255,0.06)", boxShadow: "0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.04)" }}>
      ...
      <div className="feat-content" style={{ position: "relative", padding: "1.6rem 1.4rem 1.4rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.8rem" }}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><IconStar /><span style={{ fontSize: "0.65rem", fontWeight: 800, textTransform: "uppercase", letterSpacing: 2, color: "rgba(255,255,255,0.4)" }}>Destacado</span></div><span className="feat-status" style={{ fontSize: "0.6rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px", color: sc, background: sb, padding: "4px 12px", borderRadius: 20, border: `1px solid ${sc}25`, animation: isL ? "fp2 2s ease-in-out infinite" : "none" }}>{st.text}</span></div>
        <div className="feat-matchup" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.6rem" }}>
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}><div className="feat-logo" style={{ width: 62, height: 62, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "2px solid rgba(255,255,255,0.06)", padding: 8, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.3)" }}><img src={logoUrl(match.home_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.3))" }} /></div><span className="feat-team-name" style={{ fontSize: "0.72rem", fontWeight: hw ? 800 : 600, color: hw ? "#fff" : "rgba(255,255,255,0.55)", textAlign: "center", lineHeight: 1.25, maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{match.home_name}</span>{hw && <span style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#10b981", background: "rgba(16,185,129,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(16,185,129,0.15)" }}>Ganador</span>}</div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "0 0.8rem", flexShrink: 0 }}><div className="feat-score-box" style={{ display: "flex", alignItems: "center", background: "rgba(0,0,0,0.35)", borderRadius: 14, padding: "6px 4px", border: "1px solid rgba(255,255,255,0.04)", boxShadow: "0 4px 20px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.03)" }}><span className="feat-score-num" style={{ fontSize: "1.8rem", fontWeight: 900, fontFamily: "var(--font-heading)", color: hw ? "#fff" : "rgba(255,255,255,0.6)", width: 48, textAlign: "center", lineHeight: 1 }}>{match.goles_local ?? "-"}</span><span style={{ fontSize: "1rem", color: "rgba(255,255,255,0.2)", margin: "0 2px" }}>:</span><span className="feat-score-num" style={{ fontSize: "1.8rem", fontWeight: 900, fontFamily: "var(--font-heading)", color: aw ? "#fff" : "rgba(255,255,255,0.6)", width: 48, textAlign: "center", lineHeight: 1 }}>{match.goles_visitante ?? "-"}</span></div><span style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "rgba(255,255,255,0.25)" }}>{isS ? "VS" : "FT"}</span></div>
          <div className="feat-team" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 10, maxWidth: 130 }}><div className="feat-logo" style={{ width: 62, height: 62, borderRadius: "50%", background: "rgba(255,255,255,0.04)", border: "2px solid rgba(255,255,255,0.06)", padding: 8, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.3)" }}><img src={logoUrl(match.away_logo)} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", filter: "drop-shadow(0 2px 4px rgba(0,0,0,0.3))" }} /></div><span className="feat-team-name" style={{ fontSize: "0.72rem", fontWeight: aw ? 800 : 600, color: aw ? "#fff" : "rgba(255,255,255,0.55)", textAlign: "center", lineHeight: 1.25, maxWidth: 110, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{match.away_name}</span>{aw && <span style={{ fontSize: "0.55rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "#10b981", background: "rgba(16,185,129,0.1)", padding: "2px 8px", borderRadius: 4, border: "1px solid rgba(16,185,129,0.15)" }}>Ganador</span>}</div>
        </div>
        ...
        {match.id && onVerMas && (
          <button onClick={() => onVerMas(match.id)} className="feat-btn" style={{
...
const PublicPlayerRow = ({ j }) => {
  ...
  return (
    <div className="pv-player-row">
      ...
      <span className="pv-row-pos asc-m-hide" style={{ color: pi.color, borderColor: `${pi.color}30`, background: `${pi.color}0a` }} title={pi.label}>{pi.abbr}</span>
      <div className="pv-row-stats asc-m-hide">{isGK ? (<><b style={{ color: "#f59e0b" }}>{j.goles_recibidos || 0}</b><small>GR</small><b style={{ color: "#10b981" }}>{j.vaya_invicta || 0}</b><small>VI</small></>) : (<><b style={{ color: "#ef4444" }}>{j.goles || 0}</b><small>G</small><b style={{ color: "#3b82f6" }}>{j.asistencias || 0}</b><small>A</small></>)}</div>
      <span className="pv-row-pj asc-m-hide">{j.pj || 0} <small>PJ</small></span>
    </div>
  );
};
            ...
            <div className="pv-subs-card"><h4 className="pv-subs-title"><span className="pv-subs-bench-icon">🪑</span> Suplentes <span>{subs.length}</span></h4><div className="pv-subs-grid">
              {subs.map(s => { const pi = getPosInfo(s.posicion); return (
                <div key={s.id} className="pv-sub-card" style={{ borderLeftColor: pi.color }}>
                  ...
                  <div className="pv-sub-mini-stats asc-m-hide">{pi.cat === "portero" ? (<><span style={{ color: "#f59e0b" }}>{s.goles_recibidos || 0}<small>GR</small></span><span style={{ color: "#10b981" }}>{s.vaya_invicta || 0}<small>VI</small></span></>) : (<><span style={{ color: "#ef4444" }}>{s.goles || 0}<small>G</small></span><span style={{ color: "#3b82f6" }}>{s.asistencias || 0}<small>A</small></span></>)}</div>
                </div>);
              })}
            </div></div>
          )}
...
export default function AscensoDivision() {
  ...
  const openMatchDetail = useCallback((id) => {
    if (id) navigate(`/partido/${id}/ascenso`);
  }, [navigate]);
  ...
  const loadTeamDetail = async (id) => { setExpandedTeam({ equipo: { id }, jugadores: [] }); try { const data = await safeFetch(`${API_BASE}get_equipo_detalle_ascenso.php?id=${id}`); setExpandedTeam(data); } catch (err) { setExpandedTeam(null); window.location.hash = "equipos"; } };
  const openTeam = (id) => { window.location.hash = `equipo-${id}-${expandedViewTab}`; };

  useEffect(() => {
    setLoading(true); setError(null);
    Promise.allSettled([safeFetch(`${API_BASE}get_tabla_ascenso.php`), safeFetch(`${API_BASE}get_featured_match_ascenso.php?t=${Date.now()}`), safeFetch(`${API_BASE}get_teams_ascenso.php`), safeFetch(`${API_BASE}get_sidebar_matches_ascenso.php`)]).then(r => {
  ...
  const playoffTeamIds = useMemo(() => {
    const ids = new Set();
    const sorted = [...tabla].sort((a, b) => (b.pts - a.pts) || ((b.gf - b.gc) - (a.gf - a.gc)));
    sorted.slice(0, 4).forEach(t => ids.add(t.equipo_id));
    return ids;
  }, [tabla]);
                ...
                {equipos.map(eq => { const stats = getTeamStats(eq.id); const isPO = playoffTeamIds.has(eq.id); const badge = isPO ? PLAYOFF_BADGE : null; return (
                  <div key={eq.id} className="glass-card" style={{ padding: 0, overflow: "hidden", transition: "all 0.3s ease", borderLeft: badge ? `3px solid ${badge.color}` : "3px solid transparent" }}
                    ...
                    <div style={{ padding: "1.5rem" }}>
                      ...
                      {stats && (<><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.5rem", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1rem" }}>{[{ l: "PJ", v: stats.pj, c: "var(--color-text-main)" }, { l: "G", v: stats.pg, c: "#10b981" }, { l: "E", v: stats.pe, c: "#f59e0b" }, { l: "P", v: stats.pp, c: "#ef4444" }].map((s, i) => (<div key={i} style={{ textAlign: "center" }}><div style={{ fontSize: "1.1rem", fontWeight: 800, color: s.c, fontFamily: "var(--font-heading)", lineHeight: 1 }}>{s.v}</div><div style={{ fontSize: "0.65rem", color: "var(--color-text-muted)", marginTop: "0.2rem", textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.l}</div></div>))}</div><div style={{ marginTop: "1rem", paddingTop: "0.8rem", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "0.78rem", color: "var(--color-text-muted)", fontWeight: 600 }}>Puntos</span><div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}><div style={{ width: 80, height: 4, borderRadius: 2, background: "rgba(255,255,255,0.06)", overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 2, background: "linear-gradient(90deg, #22c55e, #15803d)", width: `${Math.min((stats.pts / (tabla[0]?.pts || 1)) * 100, 100)}%`, transition: "width 0.5s ease" }} /></div><span style={{ fontSize: "1.2rem", fontWeight: 900, color: "var(--color-white)", fontFamily: "var(--font-heading)", textShadow: "0 0 10px rgba(34,197,94,0.4)" }}>{stats.pts}</span></div></div></>)}
...
@media(max-width:768px){
  ...
  .asc-main-tabs button{padding:.6rem .8rem!important;font-size:.78rem!important;gap:.3rem!important}
  .asc-m-hide{display:none!important}
  .asc-table-scroll-m{overflow-x:auto!important;-webkit-overflow-scrolling:touch!important;margin:0 -1.8rem!important;padding:0 1.8rem!important;width:calc(100% + 3.6rem)!important}
  .asc-table-m{min-width:380px!important}
  .asc-table-m th,.asc-table-m td{padding:8px 6px!important;font-size:.78rem!important}
  .asc-team-name-m{font-size:.8rem!important;max-width:110px!important;overflow:hidden!important;text-overflow:ellipsis!important}
  .asc-th-pos-m{width:32px!important}
  .asc-zona-tabs-m{max-width:100%!important}
  .asc-zona-tabs-m button{padding:.5rem .4rem!important;font-size:.68rem!important;gap:.2rem!important}
  .asc-next-teams-m{flex-direction:column!important;gap:.8rem!important}
  .asc-next-teams-m>div{width:100%!important;justify-content:center!important}
  ...
  .asc-teams-grid-m{grid-template-columns:1fr!important;gap:1rem!important}
  .asc-teams-header-m h3{font-size:1.15rem!important}
...
@media(max-width:480px){
  .asc-page-title-row h2{font-size:1.3rem!important}
  .asc-main-tabs button{font-size:.72rem!important;padding:.55rem .5rem!important}
  .asc-table-m{min-width:320px!important}
  .asc-team-name-m{font-size:.75rem!important;max-width:85px!important}
  .asc-zona-label-m{display:none!important}
  ...
  .feat-logo{width:40px!important;height:40px!important;padding:5px!important}
  .feat-score-num{font-size:1.2rem!important;width:30px!important}
  .feat-team-name{font-size:.6rem!important;max-width:65px!important}
}
...
@media(max-width:375px){
  .asc-page-title-row h2{font-size:1.1rem!important}
  .asc-main-tabs button{font-size:.62rem!important;padding:.4rem .3rem!important}
  .asc-table-m{min-width:280px!important}
  .asc-table-m th,.asc-table-m td{padding:4px 2px!important;font-size:.6rem!important}
  .asc-team-name-m{font-size:.62rem!important;max-width:60px!important}
  .feat-logo{width:34px!important;height:34px!important}
  .feat-score-num{font-size:1rem!important}
  .feat-team-name{font-size:.5rem!important;max-width:50px!important}
  .asc-teams-grid-m{grid-template-columns:1fr!important;gap:.5rem!important}
