...
import {
  ...
  CheckCircle2, Clock, Swords, ArrowRight, Lock, ArrowLeftRight, Zap,
} from "lucide-react";
...
const pairKnockoutMatches = (matchList) => {
  ...
  for(let i=0;i<matchList.length;i++){
    ...
    if(pairIdx!==-1){used.add(i);used.add(pairIdx);const m1=matchList[i],m2=matchList[pairIdx];
...
const getAvailableForPhase = (phase, allMatches, allTeams) => {
  let qualified=[];
  if(phase==="octavos") qualified=getGroupQualifiers(allMatches,allTeams);
  else if(phase==="cuartos") qualified=getPhaseWinners(allMatches,"octavos");
  else if(phase==="semis") qualified=getPhaseWinners(allMatches,"cuartos");
  const existing=[...new Set(allMatches.filter(m=>m.fase===phase).flatMap(m=>[String(m.team1_id),String(m.team2_id)]))];
...
const MatchModal = ({ match, teams, phase, allMatches, onSave, onClose }) => {
  ...
  const [form, setForm] = useState(() => {
    if (match) return { ...match, team1_id:match.team1_id||"", team2_id:match.team2_id||"", fase:match.fase||phase, grupo:match.grupo||(phase==="grupos"?"A":""), jornada:match.jornada||(isIdaVueltaPhase(match.fase)?"ida":""), penales_local:match.penales_local||"", penales_visitante:match.penales_visitante||"" };
    return { team1_id:"", team2_id:"", fecha:"", hora:"", goles_local:"", goles_visitante:"", estado:"Pendiente", fase:phase, grupo:phase==="grupos"?"A":"", jornada:isIV?"ida":"", penales_local:"", penales_visitante:"" };
  });
  ...
  const handleSave = async () => {
    const errs=validate(); setErrors(errs); setTouched({team1:true,team2:true,fecha:true,hora:true,jornada:true,goles_local:true,goles_visitante:true,grupo:true});
    if(Object.keys(errs).length>0){Swal.fire({toast:true,position:"top-end",icon:"error",title:"Corrige los errores",showConfirmButton:false,timer:2500});return;}
    setSaving(true);
    try{
      const endpoint=form.id?"copa_update_match.php":"copa_create_match.php";
      const payload={...form,grupo:currentIsGrupos?form.grupo:null,jornada:currentIsIV?form.jornada:null};
      delete payload.penales_local;delete payload.penales_visitante;
      if(currentIsIV&&form.jornada==="vuelta"&&form.penales_local!==""&&form.penales_visitante!==""){payload.penales_local=form.penales_local;payload.penales_visitante=form.penales_visitante;}
      const res=await safeFetch(`${API_BASE}${endpoint}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
      if(res.success){Swal.fire({toast:true,position:"top-end",icon:"success",title:form.id?"Partido actualizado":"Partido creado",showConfirmButton:false,timer:1800});onSave();}
...
const GroupModal = ({ teams, existingMatches, onSave, onClose }) => {
  ...
  const handleSaveAndGenerate=async()=>{
    if(!hasGroups){Swal.fire({toast:true,position:"top-end",icon:"warning",title:"Asigna al menos 2 equipos a un grupo",showConfirmButton:false,timer:2500});return;}
    if(total<teams.length){const r=await Swal.fire({title:"Equipos sin grupo",text:`${teams.length-total} sin asignar. ¿Continuar?`,icon:"warning",showCancelButton:true,confirmButtonText:"Sí",cancelButtonText:"Cancelar",background:"#1e293b",color:"#fff"});if(!r.isConfirmed)return;}
    ...
    GROUPS.forEach(g=>{
      ...
      const newPairs=pairs.filter(([t1,t2])=>!existing.some(m=>{const p1=[String(m.team1_id),String(m.team2_id)].sort().join("-");const p2=[String(t1),String(t2)].sort().join("-");return p1===p2;}));
      if(newPairs.length>0){
        const names=newPairs.map(([t1,t2])=>{const n1=teams.find(x=>String(x.id)===t1)?.nombre||"?";const n2=teams.find(x=>String(x.id)===t2)?.nombre||"?";return `${n1} vs ${n2}`;});
        preview.push(`<div style="margin-bottom:10px"><div style="font-weight:700;color:#ef4444;font-size:12px;margin-bottom:4px">Grupo ${g} — ${newPairs.length} partido${newPairs.length!==1?"s":""}</div>${names.map(n=>`<div style="font-size:11px;color:#94a3b8;padding:1px 0">• ${n}</div>`).join("")}</div>`);
      }
    });

    if(preview.length===0){Swal.fire({toast:true,position:"top-end",icon:"info",title:"Todos los partidos de grupos ya existen",showConfirmButton:false,timer:2500});return;}

    const confirm=await Swal.fire({
      title:"⚡ Auto-generar partidos de grupos",
      html:`<div style="text-align:left;max-height:300px;overflow-y:auto;padding:0 8px">${preview.join("")}</div><div style="margin-top:10px;padding:8px 12px;border-radius:8;background:rgba(168,85,247,0.08);border:1px solid rgba(168,85,247,0.15);font-size:11px;color:#a855f7;text-align:center;font-weight:600">Se crearán ${newMatchCount} partido${newMatchCount!==1?"s":""} en estado Pendiente</div>`,
      icon:"info",showCancelButton:true,confirmButtonText:"⚡ Generar",cancelButtonText:"Cancelar",
      confirmButtonColor:"#a855f7",background:"#1e293b",color:"#fff",width:480,
    });
    ...
    setGenerating(true);
    try{
      const res=await safeFetch(`${API_BASE}copa_save_groups.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({assignments})});
      if(!res.success) throw new Error(res.message||"Error al guardar grupos");
    }catch(err){Swal.fire({toast:true,position:"top-end",icon:"error",title:err.message,showConfirmButton:false,timer:2500});setGenerating(false);return;}
    ...
    GROUPS.forEach(g=>{
      ...
      const existing=existingMatches.filter(m=>m.fase==="grupos"&&m.grupo===g);
      pairs.forEach(([t1,t2])=>{
        const exists=existing.some(m=>{const p1=[String(m.team1_id),String(m.team2_id)].sort().join("-");const p2=[String(t1),String(t2)].sort().join("-");return p1===p2;});
        if(!exists) toCreate.push({fase:"grupos",grupo:g,team1_id:t1,team2_id:t2,fecha:"",hora:"",goles_local:"",goles_visitante:"",estado:"Pendiente"});
      });
    });

    setGenProgress({done:0,total:toCreate.length});
    let created=0,failed=0;
    const batch=Promise.allSettled(toCreate.map(m=>safeFetch(`${API_BASE}copa_create_match.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(m)}).then(r=>{created++;setGenProgress({done:created,total:toCreate.length});if(!r.success) throw new Error();}).catch(()=>{failed++;setGenProgress({done:created+failed,total:toCreate.length});})));
    await batch;
    setGenerating(false);

    if(failed===0){
      Swal.fire({toast:true,position:"top-end",icon:"success",title:`✅ ${created} partidos generados correctamente`,showConfirmButton:false,timer:3000});
      onSave();
    }else{
      Swal.fire({toast:true,position:"top-end",icon:"warning",title:`${created} creados, ${failed} fallidos`,showConfirmButton:false,timer:3000});
      onSave();
    }
  };

  const handleCSVImport=(e)=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=(ev)=>{const text=ev.target.result;const lines=text.split(/\r?\n/).map(l=>l.trim()).filter(Boolean);if(lines.length<2){Swal.fire({toast:true,position:"top-end",icon:"error",title:"CSV vacío",showConfirmButton:false,timer:2000});return;}const header=lines[0].toLowerCase().split(",").map(c=>c.trim());const grupoIdx=header.findIndex(c=>c==="grupo");const nombreIdx=header.findIndex(c=>c==="nombre"||c==="equipo"||c==="team");if(grupoIdx===-1||nombreIdx===-1){Swal.fire({toast:true,position:"top-end",icon:"error",title:'CSV: Grupo, Nombre',showConfirmButton:false,timer:2500});return;}const log=[];let asignados=0,noEncontrados=0;const teamMap={};teams.forEach(t=>{teamMap[t.nombre.trim().toLowerCase()]=t.id;});const na={A:[],B:[],C:[],D:[],E:[],F:[]};for(let i=1;i<lines.length;i++){const c2=lines[i].split(",").map(c=>c.trim());const g=c2[grupoIdx]?.toUpperCase();const n=c2[nombreIdx]?.trim();if(!GROUPS.includes(g)||!n){log.push(`❌ Fila ${i+1}`);continue;}const tid=teamMap[n.toLowerCase()];if(!tid){noEncontrados++;log.push(`⚠️ "${n}" no encontrado`);continue;}if(na[g].includes(String(tid))){log.push(`⚠️ "${n}" ya en G${g}`);continue;}if(na[g].length>=4){log.push(`⚠️ G${g} lleno`);continue;}na[g].push(String(tid));asignados++;log.push(`✅ "${n}" → G${g}`);}setAssignments(na);setImportLog(log);Swal.fire({toast:true,position:"top-end",icon:noEncontrados>0?"warning":"success",title:`${asignados} asignados`,showConfirmButton:false,timer:3000});};reader.readAsText(file);e.target.value="";};
      ...
      <div style={{background:"#0d1117",border:"1px solid #1e293b",borderRadius:16,width:"100%",maxWidth:760,maxHeight:"93vh",overflow:"auto",boxShadow:"0 25px 60px rgba(0,0,0,0.9)",animation:"modalSlideUp 0.28s cubic-bezier(0.16,1,0.3,1)"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:20,display:"flex",flexDirection:"column",gap:18}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {GROUPS.map(g=>{const gt=(assignments[g]||[]).map(id=>teams.find(t=>String(t.id)===id)).filter(Boolean);const full=gt.length===4;const matchCount=gt.length>=2?gt.length*(gt.length-1)/2:0;return(
...
const Sidebar = () => {
  ...
  const navItems=[{path:"/dashboard",icon:<LayoutDashboard size={18}/>,label:"Dashboard"},{path:"/matches",icon:<CalendarDays size={18}/>,label:"Partidos"},{path:"/mynews",icon:<Newspaper size={18}/>,label:"Crear Noticias"},{type:"dropdown",icon:<Shield size={18}/>,label:"Equipos",children:[{path:"/teams/primera",label:"Primera"},{path:"/teams/segunda",label:"Segunda"},{path:"/teams/tercera",label:"Tercera"}]},{path:"/admin/plantilla",icon:<Target size={18}/>,label:"Plantillas"},{path:"/posiciones",icon:<Trophy size={18}/>,label:"Posiciones"},{path:"/admin/copa",icon:<Trophy size={18}/>,label:"Copa Presidente"},{path:"/manage-news",icon:<Newspaper size={18}/>,label:"Noticias"},{path:"/users",icon:<Users size={18}/>,label:"Usuarios"},{path:"/settings",icon:<Settings size={18}/>,label:"Configuración"}];
...
const AdminCopPresidente = () => {
  ...
  const handleCreateVuelta=(idaMatch)=>{setEditingMatch({fase:activePhase,jornada:"vuelta",team1_id:String(idaMatch.team2_id),team2_id:String(idaMatch.team1_id),fecha:"",hora:"",goles_local:"",goles_visitante:"",estado:"Pendiente"});setShowMatchModal(true);};
      ...
      <main className="main-content">
        ...
        <div className="content-wrapper" style={{padding:"1.2rem"}}>
          ...
          {activePhase==="grupos"&&viewMode==="matches"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>
            {GROUPS.filter(g=>phaseMatches.some(m=>m.grupo===g)).map(g=>{const gMatches=phaseMatches.filter(m=>m.grupo===g);if(!gMatches.length)return null;return(
              <div key={g} style={{background:"rgba(255,255,255,0.01)",border:"1px solid #1e293b",borderRadius:14,overflow:"hidden"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:2,padding:6}}>
                  {gMatches.map((m,idx)=>{const fin=m.estado==="Finalizado";
