...
const DIVISIONES = [
  { key: "reservas", label: "Copa Burger King" },
];

const ManageTeamsBurgerKing = () => {
  ...
  const [seleccionesOpen, setSeleccionesOpen] = useState(false);
    ...
    const currentDivision = pathParts[2] || "reservas";
    const divisionLabel = DIVISIONES.find(d => d.key === currentDivision)?.label || "Copa Burger King";
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`${API}get_teams_burgerking.php?division=${currentDivision}`)
            .then(res => res.json())
            .then(data => {
    ...
    const addTeam = () => {
        ...
        apiPostForm(`${API}add_team_burgerking.php`, form)
            .then(res => res.json())
            .then(data => {
                setSubmitting(false);
    ...
    const saveEdit = () => {
        ...
        apiPostForm(`${API}update_team_burgerking.php`, form)
            .then(res => res.json())
            .then(data => {
    ...
    const deleteTeam = (id, nombre) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                apiPostForm(`${API}delete_team_burgerking.php`, form)
                .then(res => res.json())
                .then(data => {
                    if (data.error) {
                        Swal.fire("Error", data.error, "error");
    ...
    const navItems = [
      ...
      { path: "/mynews", icon: <CalendarDays size={20} />, label: "Crear Noticias" },
      {
         ...
         children: [
           ...
           { path: "/teams/reservas", label: "Copa Burger King" },
                                ...
                                <div className="tm-logo-section">
                                    ...
                                    {editLogo && (
                                        <button className="tm-remove-logo tm-remove-blue" onClick={() => {
                                            setEditLogo(null);
                                            setEditLogoPreview(editingTeam.logo ? `${API}${editingTeam.logo}` : null);
...
export default ManageTeamsBurgerKing;