...
const ManageMatches = () => {
  ...
  const loadMatches = () => {
    ...
    setLoading(true);
    setMatches([]); setTeams([]); setTeamMap({});
  ...
  const safeJson = async (res) => {
    ...
    if (text.trim().startsWith("<")) throw new Error("Error del servidor (PHP).");
  ...
  /* ====================================================================
   *  SEGUNDA: Determina el grupo activo según el equipo ya seleccionado
   * ==================================================================== */
  ...
  /* ====================================================================
   *  SEGUNDA: Handlers que limpian el otro equipo si cambian de grupo
   * ==================================================================== */
  const handleSelectLocal = (id) => {
    setNewLocal(id);
    if (division === "segunda" && newVisitante) {
      const lt = teamMap[id]; const vt = teamMap[newVisitante];
      if (lt?.grupo && vt?.grupo && lt.grupo.toLowerCase() !== vt.grupo.toLowerCase()) {
        setNewVisitante("");
      }
    }
  };
  ...
  const toggleFeatured = (match) => {
    ...
    Swal.fire({
      ...
      icon: isFeatured ? "question" : "warning",
      showCancelButton: true,
      confirmButtonText: isFeatured ? "Sí, quitar" : "Sí, destacar",
      cancelButtonText: "Cancelar",
      confirmButtonColor: isFeatured ? "#ef4444" : "#e2b340",
      ...
      setMatches(prev => prev.map(m => {
        if (!isFeatured) {
          return String(m.id) === matchId ? { ...m, featured: 1, destacado: 1 } : { ...m, featured: 0, destacado: 0 };
        } else {
          return String(m.id) === matchId ? { ...m, featured: 0, destacado: 0 } : m;
        }
      }));
      setSubmitting(false);
      const form = new FormData();
      form.append("match_id", match.id);
      form.append("featured", isFeatured ? "0" : "1");
      fetch(getEndpoints().toggleFeatured, { method: "POST", body: form })
        ...
        .then(data => {
          if (data.error) {
            setMatches(prev => prev.map(m => String(m.id) === matchId ? { ...m, featured: isFeatured ? 1 : 0, destacado: isFeatured ? 1 : 0 } : m));
            Swal.fire("Error", data.error, "error");
          } else {
            Swal.fire({ icon: "success", title: isFeatured ? "Destacado quitado" : "Partido destacado", toast: true, position: "top-end", timer: 2000, showConfirmButton: false });
          }
        })
        .catch(err => {
          if (err.message === "PHP_MISSING") {
            Swal.fire({ icon: "warning", title: "Archivo PHP no encontrado", html: `<p style="color:#94a3b8;font-size:13px;margin:0 0 6px">No se encontró <strong style="color:#fbbf24">${getEndpoints().toggleFeatured.split("/").pop()}</strong></p><p style="color:#64748b;font-size:12px;margin:0">El cambio es visual. Crea el archivo PHP para persistirlo en la BD.</p>`, timer: 5000, showConfirmButton: false });
          }
        });
    });
  };
  ...
  /* ====================================================================
   *  SEGUNDA: Filtrar equipos por grupo — solo mismo grupo vs mismo grupo
   * ==================================================================== */
  const filteredLocal = teams.filter(t => {
    ...
    if (division === "segunda" && newVisitante) {
      const vGrupo = (teamMap[newVisitante]?.grupo || "").toLowerCase();
      const tGrupo = (t.grupo || "").toLowerCase();
      if (vGrupo && tGrupo && vGrupo !== tGrupo) return false;
    }
    return true;
  });
  const filteredVisitante = teams.filter(t => {
    ...
    if (division === "segunda" && newLocal) {
      const lGrupo = (teamMap[newLocal]?.grupo || "").toLowerCase();
      const tGrupo = (t.grupo || "").toLowerCase();
      if (lGrupo && tGrupo && lGrupo !== tGrupo) return false;
    }
    return true;
  });
  ...
  /* ====================================================================
   *  SEGUNDA: Badge de grupo para el dropdown y la preview
   * ==================================================================== */
  const GrupoBadge = ({ grupo, size = "sm" }) => {
    ...
    const isEast = g === "east";
    const s = size === "lg"
      ? { padding: "3px 10px", fontSize: "10px", gap: "4px", borderRadius: "5px" }
      : { padding: "2px 7px", fontSize: "9px", gap: "3px", borderRadius: "4px" };
    return (
      <span style={{
        display: "inline-flex", alignItems: "center", gap: s.gap, padding: s.padding,
        borderRadius: s.borderRadius, fontSize: s.fontSize, fontWeight: 800, letterSpacing: "0.3px",
    ...
    const renderGrouped = () => {
      ...
      return (
        <>
          {east.length > 0 && (
            <>
              <div className="cs-group-sep cs-group-sep-east"><span style={{ width: 6, height: 6, borderRadius: "50%", background: "#3b82f6" }} />Grupo Este</div>
              {east.map(renderOpt)}
            </>
          )}
          {west.length > 0 && (
            <>
              <div className="cs-group-sep cs-group-sep-west"><span style={{ width: 6, height: 6, borderRadius: "50%", background: "#f97316" }} />Grupo Oeste</div>
              {west.map(renderOpt)}
            </>
          )}
        ...
        {isOpen && (
          <div className="cs-dropdown cs-dropdown-lg animate__animated animate__fadeInDown">
            ...
            <div className="cs-options cs-options-lg">
              {filtered.length === 0 ? (
                <div className="cs-empty cs-empty-lg">
                  {isSegunda && activeGrupo ? `Sin equipos en Grupo ${activeGrupo === "East" ? "Este" : "Oeste"}` : "Sin resultados"}
                </div>
    ...
    <div className={`admin-layout ${!sidebarOpen ? "sidebar-closed" : ""}`}>
            ...
            <div ref={dropdownRef} style={{ position: "relative" }}>
              ...
              {dropdownOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, zIndex: 100, background: "#1e293b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", overflow: "hidden", minWidth: "190px", boxShadow: "0 12px 40px rgba(0,0,0,0.5)", animation: "ddFadeIn 0.15s ease-out" }}>
                  {DIVISIONES.map(d => (
                    <button key={d.value} onClick={() => { setDivision(d.value); setDropdownOpen(false); }} style={{ display: "flex", alignItems: "center", gap: "10px", width: "100%", padding: "12px 18px", border: "none", background: division === d.value ? "rgba(59,130,246,0.15)" : "transparent", color: division === d.value ? "#60a5fa" : "#94a3b8", fontSize: "13px", fontWeight: division === d.value ? 700 : 500, cursor: "pointer", textAlign: "left" }}>
          ...
          <div className="table-container">
            ...
            {loading ? (
              ...
              <table className="data-table">
                <thead>
                  <tr><th>Fecha</th><th>Local</th><th style={{ textAlign: "center" }}>Marcador</th><th>Visitante</th><th>Estado</th><th>Acciones</th></tr>
                </thead>
                <tbody>
                  {filteredMatches.map(match => {
                    ...
                    return (
                      <tr key={match.id}>
                        ...
                        <td><div style={{ display: "flex", alignItems: "center", gap: "10px" }}><img src={escLocal || fallbackImg} alt="" onError={e => { e.target.src = fallbackImg; }} style={{ width: "32px", height: "32px", objectFit: "contain", borderRadius: "6px", background: "#fff", padding: "2px", flexShrink: 0 }} /><span style={{ fontWeight: 600 }}>{match.local_nombre || "—"}</span></div></td>
                        ...
                        <td><div style={{ display: "flex", alignItems: "center", gap: "10px" }}><img src={escVisit || fallbackImg} alt="" onError={e => { e.target.src = fallbackImg; }} style={{ width: "32px", height: "32px", objectFit: "contain", borderRadius: "6px", background: "#fff", padding: "2px", flexShrink: 0 }} /><span style={{ fontWeight: 600 }}>{match.visitante_nombre || "—"}</span></div></td>
                        <td><span className={`status ${isFin ? "done" : "pending"}`}>{match.status || "Pendiente"}</span></td>
                        <td>
        ...
        <div className="nm-overlay" onClick={e => { if (e.target === e.currentTarget) setShowNewMatch(false); }}>
          <div className="nm-card nm-card-wide animate__animated animate__fadeInUp">
            ...
            <div className="nm-body nm-body-wide">
              {/* BANNER RESTRICCIÓN SEGUNDA */}
              {division === "segunda" && (
                <div className="nm-grupo-banner" style={{
                  padding: "12px 18px", borderRadius: "12px", marginBottom: "22px",
                  background: activeGrupo
                    ? (activeGrupo.toLowerCase() === "east" ? "rgba(59,130,246,0.08)" : "rgba(249,115,22,0.08)")
                    : "rgba(255,255,255,0.025)",
                  border: `1px solid ${activeGrupo
                    ? (activeGrupo.toLowerCase() === "east" ? "rgba(59,130,246,0.18)" : "rgba(249,115,22,0.18)")
                    : "rgba(255,255,255,0.06)"}`,
                  display: "flex", alignItems: "center", gap: "10px",
                  fontSize: "13px", fontWeight: 600, transition: "all 0.3s",
                  color: activeGrupo
                    ? (activeGrupo.toLowerCase() === "east" ? "#60a5fa" : "#fb923c")
                    : "#64748b",
                }}>
                  <Shield size={16} style={{ flexShrink: 0 }} />
                  <span>
                    {activeGrupo
                      ? <>Filtrado: solo equipos del <strong>Grupo {activeGrupo === "East" ? "Este" : "Oeste"}</strong></>
                      : "Los partidos son entre equipos del mismo grupo (Este vs Este / Oeste vs Oeste)"}
                  </span>
                </div>
              )}
              ...
              {(localTeam || visitanteTeam) && (
                <div className="nm-preview nm-preview-lg">
                  <div className="nm-preview-side">
                    {localTeam ? (<>
                      <div className="nm-preview-logo-wrap nm-preview-logo-wrap-lg"><img src={`${API}${localTeam.logo}`} alt="" onError={e => { e.target.src = fallbackImg; }} className="nm-preview-logo" /></div>
                      <span className="nm-preview-name nm-preview-name-lg">{localTeam.nombre}</span>
                      {division === "segunda" && <GrupoBadge grupo={localTeam.grupo} size="lg" />}
                      {!division === "segunda" && localTeam.ciudad && <span className="nm-preview-city nm-preview-city-lg">{localTeam.ciudad}</span>}
                    </>) : (<div className="nm-preview-empty"><div className="nm-preview-logo-wrap nm-preview-logo-wrap-lg nm-preview-logo-empty"><span>?</span></div><span className="nm-preview-name nm-preview-name-lg" style={{ color: "#1e293b" }}>Sin seleccionar</span></div>)}
                  </div>
                  ...
                  <div className="nm-preview-side">
                    {visitanteTeam ? (<>
                      <div className="nm-preview-logo-wrap nm-preview-logo-wrap-lg"><img src={`${API}${visitanteTeam.logo}`} alt="" onError={e => { e.target.src = fallbackImg; }} className="nm-preview-logo" /></div>
                      <span className="nm-preview-name nm-preview-name-lg">{visitanteTeam.nombre}</span>
                      {division === "segunda" && <GrupoBadge grupo={visitanteTeam.grupo} size="lg" />}
                      {!division === "segunda" && visitanteTeam.ciudad && <span className="nm-preview-city nm-preview-city-lg">{visitanteTeam.ciudad}</span>}
                    </>) : (<div className="nm-preview-empty"><div className="nm-preview-logo-wrap nm-preview-logo-wrap-lg nm-preview-logo-empty"><span>?</span></div><span className="nm-preview-name nm-preview-name-lg" style={{ color: "#1e293b" }}>Sin seleccionar</span></div>)}
                  </div>
                </div>
              )}
              ...
              {division === "segunda" && newLocal && newVisitante && (() => {
                const lg = (teamMap[newLocal]?.grupo || "").toLowerCase();
                const vg = (teamMap[newVisitante]?.grupo || "").toLowerCase();
                if (lg && vg && lg !== vg) return <div className="nm-warning nm-warning-lg">Los equipos deben ser del mismo grupo</div>;
                return null;
              })()}
            </div>
      ...
      <style>{`
        ...
        @media (max-width: 640px) {
          ...
          .result-action-btn span, .mm-reset-single-btn span, .mm-reset-all-btn span, .mm-star-btn span { display: none; }
        }
      `}</style>
    </div>
  );
