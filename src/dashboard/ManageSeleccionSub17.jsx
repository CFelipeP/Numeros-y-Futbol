...
export default function ManageSeleccionsub17() {
  ...
  const loadAll = () => {
    setLoading(true);
    Promise.all([
      apiFetch(`${API}crud_partidos_seleccion_sub17.php`).then(r => r.json()),
      apiFetch(`${API}crud_jugadores_seleccion_sub17.php`).then(r => r.json()),
      apiFetch(`${API}crud_cuerpo_tecnico_sub17.php`).then(r => r.json()),
    ]).then(([pd, jd, sd]) => {
      ...
      <main className="main-content">
        <header className="top-bar">
          ...
          <div style={{ fontSize: "13px", fontWeight: 600, color: "#64748b", display: "flex", alignItems: "center", gap: "8px" }}>
            <Shield size={16} /> Selección Sub-17 Salvadoreña
          </div>
        </header>

        <div className="content-wrapper">
          <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "1.2rem", flexWrap: "wrap" }}>
            <h1 className="admin-title" style={{ margin: 0 }}>Selección Sub-17 Salvadoreña</h1>
          </div>
            ...
            {[
              ...
              <button key={t.key} onClick={() => setActiveTab(t.key)}
                style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", padding: "10px 14px", borderRadius: "9px", border: "none", cursor: "pointer", fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: "12px", transition: "0.2s",
                  background: activeTab === t.key ? "linear-gradient(135deg,rgba(168,85,247,0.18),rgba(168,85,247,0.06))" : "transparent",
                  color: activeTab === t.key ? "#a855f7" : "#64748b" }}>
          ...
          {loading ? (
            <div style={{ textAlign: "center", padding: "3rem", color: "#64748b" }}>
              <div style={{ width: 36, height: 36, borderRadius: "50%", border: "3px solid rgba(255,255,255,0.08)", borderTopColor: "#a855f7", animation: "mmSpin 0.8s linear infinite", margin: "0 auto 1rem" }} />
              Cargando datos...
            </div>
          ) : (
            <>
              {activeTab === "partidos" && <PartidosTabFemenina partidos={partidos} onReload={loadAll} />}
              {activeTab === "jugadores" && <JugadoresTabsub17 jugadores={jugadores} onReload={loadAll} />}
...
function PartidosTabFemenina({ partidos, onReload }) {
  ...
  const save = () => {
    ...
    apiPost(`${API}crud_partidos_seleccion_sub17.php`, payload).then(r => r.json()).then(d => {
  ...
  const deletePartido = (id, rival) => {
    Swal.fire({ title: "¿Eliminar partido?", text: `vs ${rival}`, icon: "warning", showCancelButton: true, confirmButtonText: "Sí, eliminar", confirmButtonColor: "#d33" })
      .then(r => { if (r.isConfirmed) { apiPost(`${API}crud_partidos_seleccion_sub17.php`, { action: "delete", id }).then(r => r.json()).then(d => { if (d.success) { Swal.fire({ icon: "success", title: "Eliminado", toast: true, position: "top-end", timer: 1500, showConfirmButton: false }).then(onReload); } }).catch(() => Swal.fire("Error", "Error", "error")); } });
  };
...
function JugadoresTabsub17({ jugadores, onReload }) {
  const [showForm, setShowForm] = useState(false);
  const [editId, setEditId] = useState(null);
  ...
  const handleUpload = async (e) => {
    ...
    setUploading(true);
    try {
      const res = await fetch(`${API}upload_seleccion_foto_sub17.php`, { method: "POST", headers: getAuthHeaders(), body: fd });
  ...
  const save = () => {
    ...
    const payload = { ...form, action: editId ? "update" : "create", id: editId };
    if (editId) payload.id = editId;
    apiPost(`${API}crud_jugadores_seleccion_sub17.php`, payload).then(r => r.json()).then(d => {
      if (d.success) { Swal.fire({ icon: "success", title: editId ? "Actualizado" : "Creado", toast: true, position: "top-end", timer: 1500, showConfirmButton: false }).then(onReload); setShowForm(false); }
      else Swal.fire("Error", d.error || "Error", "error");
    }).catch(() => Swal.fire("Error", "Error de conexión", "error"));
  };
  ...
  const deleteJugador = (id, nombre) => {
    Swal.fire({ title: "¿Eliminar jugador?", text: nombre, icon: "warning", showCancelButton: true, confirmButtonText: "Sí, eliminar", confirmButtonColor: "#d33" })
      .then(r => { if (r.isConfirmed) { apiPost(`${API}crud_jugadores_seleccion_sub17.php`, { action: "delete", id }).then(r => r.json()).then(d => { if (d.success) { Swal.fire({ icon: "success", title: "Eliminado", toast: true, position: "top-end", timer: 1500, showConfirmButton: false }).then(onReload); } }).catch(() => Swal.fire("Error", "Error", "error")); } });
  };
  ...
  const downloadTemplate = () => {
    ...
    a.href = url; a.download = "plantilla_seleccion_sub17.csv";
  ...
  const importPlayers = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}importar_jugadores_seleccion_sub17.php`, { method: "POST", body: fd, headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
      const d = await res.json();
      if (d.success) {
        closeImport();
...
function TecnicoTabFemenina({ staff, onReload }) {
  ...
  const save = () => {
    ...
    const payload = { ...form, action: editId ? "update" : "create", id: editId };
    if (editId) payload.id = editId;
    apiPost(`${API}crud_cuerpo_tecnico_sub17.php`, payload).then(r => r.json()).then(d => {
      if (d.success) { Swal.fire({ icon: "success", title: editId ? "Actualizado" : "Creado", toast: true, position: "top-end", timer: 1500, showConfirmButton: false }).then(onReload); setShowForm(false); }
      else Swal.fire("Error", d.error || "Error", "error");
    }).catch(() => Swal.fire("Error", "Error de conexión", "error"));
  };
  ...
  const deleteStaff = (id, nombre) => {
    Swal.fire({ title: "¿Eliminar miembro?", text: nombre, icon: "warning", showCancelButton: true, confirmButtonText: "Sí, eliminar", confirmButtonColor: "#d33" })
      .then(r => { if (r.isConfirmed) { apiPost(`${API}crud_cuerpo_tecnico_sub17.php`, { action: "delete", id }).then(r => r.json()).then(d => { if (d.success) { Swal.fire({ icon: "success", title: "Eliminado", toast: true, position: "top-end", timer: 1500, showConfirmButton: false }).then(onReload); } }).catch(() => Swal.fire("Error", "Error", "error")); } });
  };
        ...
        <tbody>
          {staff.length === 0 ? (
            ...
            <tr key={s.id}>
              ...
              <td style={{ color: "#a855f7", fontWeight: 600 }}>{s.rol || "—"}</td>
