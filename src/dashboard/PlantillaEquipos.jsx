...
function normalizarPosicion(v) {
  ...
  const map = {
    ...
    dfc: "central",
    lateral: "lateral_derecho",
    li: "lateral_izquierdo",
    ld: "lateral_derecho",

    // mediocampo (AQUÍ ESTABA EL ERROR)
    centrocampista: "medio_central",
    medio: "medio_central",
    mediocampista: "medio_central",
    mediocampo: "medio_central",
    "medio campo": "medio_central",
    mc: "medio_central",
    mcd: "medio_defensivo",
    mco: "medio_ofensivo",
    midfielder: "medio_central",

    // extremos
    extremo: "extremo_derecho",
    ei: "extremo_izquierdo",
    ed: "extremo_derecho",
    winger: "extremo_derecho",
...
// ─── PlayerCard (vista plantilla) ────────────────────────────────────────────
const PlayerCard = memo(function PlayerCard({ j, onStats, onEdit, onDelete, onSwap }) {
  ...
  return (
    <div className={"pl-card" + (j.es_titular ? " pl-card-tit" : "")}>
      ...
      {/* Badge titular */}
      <div className={"pl-card-badge" + (j.es_titular ? " tit" : " sup")}>
...
export default function PlantillaAdmin() {
  ...
  // ─── NUEVO: Modal de cambio ────────────────────────────────────────────────
  // swapPlayer = jugador seleccionado para hacer el cambio
  // swapMode   = "salir" (titular → elegir quién entra) | "entrar" (suplente → elegir quién sale)
  ...
  useEffect(() => {
    ...
    const savedId = localStorage.getItem("admin_plantilla_team_" + division) || "";
    setLoading(true);
    setEquipos([]);
    setPlantilla(null);
  ...
  const loadPlantilla = useCallback(async (id) => {
    ...
    try {
      ...
      if (r.data.success) {
        ...
        setPlantilla({ ...r.data, jugadores });
        if (r.data.equipo?.formacion) setFormacion(r.data.equipo.formacion);
      } else {
        Swal.fire({ background:"#1e293b", color:"#fff", icon:"warning", title: r.data.error || "No se pudo cargar" });
      }
    } catch (err) {
      Swal.fire({ background:"#1e293b", color:"#fff", icon:"error", title: err.response?.status === 404 ? "Archivo no encontrado" : "Error de conexion" });
  ...
  const titularesCount = useMemo(
    () => plantilla?.jugadores?.filter(j => j.es_titular).length || 0,
    [plantilla]
  );
  ...
  const openSwap = useCallback((jugador) => {
    ...
    // Si es titular → elegimos quién entra a reemplazarlo
    // Si es suplente → elegimos quién sale para que entre él
    setSwapMode(jugador.es_titular ? "salir" : "entrar");
    setSwapModal(true);
  }, []);
  ...
  // 🚫 validación simple (misma posición opcional)
  ...
  // 🔥 UI inmediata (swap real)
  setPlantilla(prev => ({
    ...prev,
    jugadores: prev.jugadores.map(j => {
      if (j.id === swapPlayer.id) return { ...j, es_titular: otro.es_titular };
      if (j.id === otro.id)       return { ...j, es_titular: swapPlayer.es_titular };
      return j;
    })
  }));
  ...
  try {
    const r = await axios.post(ep.crud, {
      ...
      changes: [
        { id: swapPlayer.id, es_titular: otro.es_titular ? 1 : 0 },
        { id: otro.id,       es_titular: swapPlayer.es_titular ? 1 : 0 }
      ]
    });
    ...
    if (!r.data.success) {
      ...
      Swal.fire({
        ...
        color:"#fff",
        icon:"error",
        title:r.data.error || "Error en cambio"
      });
    } else {
      // ✅ feedback limpio
      Swal.fire({
...
}, [swapPlayer, equipoId, division, loadPlantilla]);
  ...
  const openCreate = useCallback(() => {
    setForm({ nombre:"", posicion:"centrodelantero", numero_camiseta:"", edad:"", nacionalidad:"", foto:"", es_titular:false });
    setEditPlayer(null);
    setModal("player");
  }, []);
  ...
  const deletePlayer = useCallback(async (j) => {
    const ok = await Swal.fire({ background:"#1e293b", color:"#fff", title:"Eliminar a " + j.nombre + "?", icon:"warning", showCancelButton:true, confirmButtonColor:"#ef4444", confirmButtonText:"Eliminar", cancelButtonText:"Cancelar" });
  ...
  const saveFormation = useCallback(async () => {
    ...
    try {
      const r = await axios.post(ep.crud, {
        action: "save_formation", equipo_id: equipoId, formacion,
        titulares: JSON.stringify(starters.map(s => ({ id: s.id, x: s.px, y: s.py })))
      });
      if (r.data.success) {
        Swal.fire({ toast:true, position:"top-end", icon:"success", title:"Formacion guardada", showConfirmButton:false, timer:1500, background:"#1e293b", color:"#fff" });
        loadPlantilla(equipoId);
      }
    } catch {
      Swal.fire({ background:"#1e293b", color:"#fff", icon:"error", title:"No se pudo guardar" });
    } finally { setSaving(false); }
  ...
  const parseCSVPreview = useCallback((text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (row.posicion) row.posicion_original = row.posicion;
      if (row.posicion) {
  ...
  const downloadTemplate = useCallback(() => {
    const t = `nombre,posicion,numero_camiseta,edad,nacionalidad,es_titular,pj,goles,asistencias,goles_penal,goles_cabeza,goles_tiro_libre,tarjetas_amarillas,tarjetas_rojas,minutos_jugados,goles_recibidos,vaya_invicta\nJuan Perez,centrodelantero,9,24,Salvadoreno,1,13,9,2,0,0,3,0,0,980,0,0\nCarlos Lopez,medio_central,10,22,Salvadoreno,0,15,5,8,0,0,2,4,1,1300,0,0`;
    ...
    <div className={"admin-layout" + (sidebarOpen ? " sidebar-open" : "")}>
      ...
      <main className="main-content">
        ...
        <div className="pl-page">
          ...
          {/* Resumen titulares (solo informativo, sin lógica de bloqueo) */}
          {plantilla && (
            <div className="pl-tit-counter">
              <div className="pl-tit-dots">
                {Array.from({ length: 11 }).map((_, i) => (
                  <div key={i} className={"pl-tit-dot" + (i < titularesCount ? " filled" : "")} />
                ))}
              </div>
              <span className="pl-tit-label">{titularesCount}/11 titulares</span>
          ...
          {plantilla && (
            <div className="pl-team-card">
              <div className="pl-tc-logo"><img src={API + plantilla.equipo.logo} alt="" /></div>
              <div className="pl-tc-info"><h3>{plantilla.equipo.nombre}</h3><p>{[plantilla.equipo.ciudad, plantilla.equipo.estadio].filter(Boolean).join(" - ")}</p></div>
              <div className="pl-tc-stats">
                <div className="pl-tc-stat"><b>{total}</b><span>Jugadores</span></div>
                <div className="pl-tc-div" />
                <div className="pl-tc-stat"><b className="text-amber">{titularesCount}</b><span>Titulares</span></div>
                <div className="pl-tc-div" />
                <div className="pl-tc-stat"><b className="text-slate">{total - titularesCount}</b><span>Suplentes</span></div>
              </div>
            </div>
          )}
          ...
          {!loading && plantilla && tab === "plantilla" && (
            total === 0 ? (
              ...
              <div>
                ...
                {Object.keys(catCfg).map(cat => <PosSection key={cat} cat={cat} jugadores={groups[cat]} {...handlers} />)}
              </div>
            )
          )}
            ...
            <div>
              ...
              <div className="pl-pitch">
                ...
                {starters.map((s, idx) => {
                  ...
                  return (
                    <div key={s.id || idx} className="pl-pitch-player" style={{ left: s.px + "%", top: s.py + "%" }}
                      onClick={() => openSwap(s)} title="Gestionar cambio">
                      <div className="pl-pitch-dot" data-color={pi.color}>{s.foto ? <img src={API + s.foto} alt="" /> : <span>{s.numero_camiseta || ""}</span>}</div>
                      <div className="pl-pitch-name">{s.nombre.split(" ").pop()}</div>
                    </div>
                  );
                })}
                {starters.length < 11 && <div className="pl-pitch-msg">{starters.length}/11 — agrega más titulares</div>}
              </div>

              {/* Suplentes clickeables para cambio */}
              {subs.length > 0 && (
                <div className="pl-subs-card">
                  <h4 className="pl-subs-title">Suplentes ({subs.length}) — clic para gestionar cambio</h4>
      ...
      {/* ─────────────────────────────────────────────────────────────────────
          MODAL DE CAMBIO
          Si swapMode === "salir": el jugador seleccionado es titular, elegimos quién entra
          Si swapMode === "entrar": el jugador seleccionado es suplente, elegimos quién sale
      ──────────────────────────────────────────────────────────────────────── */}
        ...
        <div className="pl-modal-bg" onClick={closeSwap}>
          <div className="pl-modal pl-swap-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="pl-modal-body">
              ...
              {/* Lista de candidatos: lo opuesto al modo */}
              <div className="pl-swap-list">
                {(swapMode === "salir"
                  ? plantilla.jugadores.filter(j => !j.es_titular && j.id !== swapPlayer.id)
                  : plantilla.jugadores.filter(j => j.es_titular  && j.id !== swapPlayer.id)
                ).map(j => {
                  const pi = getPosInfo(j.posicion);
                  return (
                ...
                {(swapMode === "salir"
                  ...
                  <div className="pl-swap-empty">
                    {swapMode === "salir" ? "No hay suplentes disponibles" : "No hay otros titulares"}
                  </div>
                )}
              </div>
            </div>
      ...
      {importModal && (
        <div className="pl-modal-bg" onClick={closeImport}>
          <div className="pl-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 740 }}>
            ...
            <div className="pl-modal-body">
              <div style={{ marginBottom:16 }}><button className="pl-dl-btn" onClick={downloadTemplate}><Download size={13} /> Descargar plantilla CSV</button></div>
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                ...
                <textarea value={csvText} onChange={e => { setCsvText(e.target.value); if (e.target.value.trim()) parseCSVPreview(e.target.value); else setCsvPreview([]); }}
                  ...
                  <div style={{ maxHeight:220, overflow:"auto", borderRadius:10, border:"1px solid rgba(255,255,255,0.04)" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11, fontFamily:"monospace" }}>
                      <thead><tr style={{ background:"rgba(255,255,255,0.04)", position:"sticky", top:0 }}>
                        {["#","Nombre","Pos","Original","#","Edad","Nac","Tit","PJ","Gol","Asis"].map((h,i) => (
                        ...
                        {csvPreview.map((r, i) => {
                          ...
                          return (
                            <tr key={i} style={{ background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                              ...
                              <td style={{ padding:"4px 6px", textAlign:"center", color: r.es_titular === "1" ? "#fbbf24" : "#334155", fontWeight:700 }}>{r.es_titular === "1" ? "SI" : "NO"}</td>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:20, paddingTop:16, borderTop:"1px solid rgba(255,255,255,0.04)" }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  ...
                  <button className="pl-save-btn" onClick={importPlayers} disabled={importing || csvPreview.length === 0}><Upload size={14} />{importing ? "Importando..." : "Importar " + csvPreview.length}</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pl-modal-bg" onClick={() => setModal(null)}>
          <div className="pl-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="pl-modal-body">
              ...
              <div className="pl-field"><label>Nombre completo <span className="req">*</span></label><input className="pl-input" value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} placeholder="Juan Perez" /></div>
              <div className="pl-field"><label>Posicion</label>
              ...
              <div className="pl-row2">
                <div className="pl-field"><label>Dorsal <span className="text-slate">(1-99)</span></label><input className="pl-input" type="number" value={form.numero_camiseta} onChange={e => setForm(f => ({ ...f, numero_camiseta: e.target.value }))} placeholder="10" min="1" max="99" /></div>
                <div className="pl-field"><label>Edad <span className="text-slate">(16-50)</span></label><input className="pl-input" type="number" value={form.edad} onChange={e => setForm(f => ({ ...f, edad: e.target.value }))} placeholder="20" min="16" max="50" /></div>
              </div>
              <div className="pl-field"><label>Nacionalidad</label><input className="pl-input" value={form.nacionalidad} onChange={e => setForm(f => ({ ...f, nacionalidad: e.target.value }))} placeholder="Salvadoreno" /></div>
              <div className="pl-tit-row">
                <label style={{ marginBottom:0 }}>Estado</label>
                <div className="pl-tit-btns">
                  <button type="button" className={"pl-tit-opt" + (!form.es_titular ? " active" : "")} onClick={() => setForm(f => ({ ...f, es_titular:false }))}><User size={14} /> Suplente</button>
                  <button type="button" className={"pl-tit-opt tit" + (form.es_titular ? " active" : "")} onClick={() => setForm(f => ({ ...f, es_titular:true }))}>
...
/* ── Contador titulares (solo informativo) ── */
...
/* ── Badge TIT / SUP en PlayerCard ── */
.pl-card-badge{font-size:8px;font-weight:800;letter-spacing:.1em;padding:3px 8px;border-radius:6px;flex-shrink:0;text-transform:uppercase}
.pl-card-badge.tit{background:rgba(34,197,94,.1);color:#22c55e;border:1px solid rgba(34,197,94,.2)}
.pl-card-badge.sup{background:rgba(255,255,255,.03);color:#334155;border:1px solid rgba(255,255,255,.04)}
...
.pl-sub-item[data-color="#f59e0b"]{border-left:2px solid rgba(245,158,11,.3)}.pl-sub-item[data-color="#3b82f6"]{border-left:2px solid rgba(59,130,246,.3)}.pl-sub-item[data-color="#10b981"]{border-left:2px solid rgba(16,185,129,.3)}.pl-sub-item[data-color="#ef4444"]{border-left:2px solid rgba(239,68,68,.3)}.pl-sub-item[data-color="#059669"]{border-left:2px solid rgba(5,150,105,.3)}.pl-sub-item[data-color="#6ee7b7"]{border-left:2px solid rgba(110,231,183,.3)}.pl-sub-item[data-color="#f87171"]{border-left:2px solid rgba(248,113,113,.15)}.pl-sub-item[data-color="#f97316"]{border-left:2px solid rgba(249,115,22,.3)}.pl-sub-item[data-color="#64748b"]{border-left:2px solid rgba(100,116,139,.15)}
...
.pl-card.pl-card-tit{border-color:rgba(34,197,94,.08)!important;background:rgba(34,197,94,.025)!important}
...
.pl-card-num[data-color="#f59e0b"]{color:#fbbf24}.pl-card-num[data-color="#3b82f6"]{color:#60a5fa}.pl-card-num[data-color="#10b981"]{color:#34d399}.pl-card-num[data-color="#ef4444"]{color:#f87171}.pl-card-num[data-color="#059669"]{color:#34d399}.pl-card-num[data-color="#6ee7b7"]{color:#6ee7b7}.pl-card-num[data-color="#f87171"]{color:#f87171}.pl-card-num[data-color="#f97316"]{color:#fb923c}.pl-card-num[data-color="#64748b"]{color:#94a3b8}
...
.pl-tit-opt:hover{background:rgba(100,116,139,.06)!important;color:#94a3b8!important}
.pl-tit-opt.tit.active{background:rgba(34,197,94,.08)!important;color:#22c55e!important;border-color:rgba(34,197,94,.15)!important}
