...
function countTitulares(PDO $db, int $equipo_id, ?int $excludeId=null): int {
    $sql = "SELECT COUNT(*) FROM jugadores_ascenso WHERE equipo_id=? AND es_titular=1";
    $params = [$equipo_id];
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $eq = $conn->prepare("SELECT * FROM equipos_ascenso WHERE id=?");
    ...
    $st = $conn->prepare("
        ...
        LEFT JOIN estadisticas_jugadores_ascenso e ON e.jugador_id=j.id AND e.temporada=:temporada
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $conn->prepare("DELETE FROM jugadores_ascenso WHERE id=?")->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($num !== null) {
            $chk = $conn->prepare("SELECT COUNT(*) FROM jugadores_ascenso WHERE equipo_id=? AND numero_camiseta=?");
        ...
        if ($es_titular) {
            $cnt = $conn->prepare("SELECT COUNT(*) FROM jugadores_ascenso WHERE equipo_id=? AND es_titular=1");
        ...
        $conn->prepare("INSERT INTO jugadores_ascenso (equipo_id,nombre,posicion,numero_camiseta,edad,nacionalidad,foto,es_titular) VALUES (?,?,?,?,?,?,?,?)")
            ->execute([$equipo_id,$nombre,$posicion,$num,$edad,$nac,$foto,$es_titular]);
        $jugador_id = $conn->lastInsertId();

        $conn->prepare("INSERT IGNORE INTO estadisticas_jugadores_ascenso (jugador_id,temporada) VALUES (?,?)")
    ...
    if ($action === 'update') {
        ...
        if ($num !== null) {
            $chk = $conn->prepare("SELECT COUNT(*) FROM jugadores_ascenso WHERE equipo_id=? AND numero_camiseta=? AND id!=?");
        ...
        if ($es_titular) {
            $cnt = $conn->prepare("SELECT COUNT(*) FROM jugadores_ascenso WHERE equipo_id=? AND es_titular=1 AND id!=?");
        ...
        $conn->prepare("UPDATE jugadores_ascenso SET nombre=?,posicion=?,numero_camiseta=?,edad=?,nacionalidad=?,foto=?,es_titular=? WHERE id=?")
    ...
    if ($action === 'toggle_bulk') {
        ...
        $cur = $conn->prepare("SELECT id, es_titular FROM jugadores_ascenso WHERE equipo_id=?");
        ...
        $stmt = $conn->prepare("UPDATE jugadores_ascenso SET es_titular=? WHERE id=? AND equipo_id=?");
        foreach ($changes as $ch) {
    ...
    if ($action === 'update_stats') {
        ...
        $conn->prepare("
            INSERT INTO estadisticas_jugadores_ascenso
