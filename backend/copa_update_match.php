...
try {
    ...
    if ($fase === 'grupos') {
        ...
        $teamReuse = $pdo->prepare("
            SELECT COUNT(*) FROM partidos_copa
            WHERE id <> ? AND fase = ? AND llave != ?
              AND (equipo_local_id IN (?, ?) OR equipo_visitante_id IN (?, ?))
        ");
        $teamReuse->execute([$id, $fase, ($llave ?? 0), $team1, $team2, $team1, $team2]);
    ...
    if ($fase !== 'grupos') {
        $teamReuse = $pdo->prepare("SELECT COUNT(*) FROM partidos_copa WHERE id <> ? AND fase = ? AND llave != ? AND (equipo_local_id IN (?, ?) OR equipo_visitante_id IN (?, ?))");
        $teamReuse->execute([$id, $fase, ($llave ?? 0), $team1, $team2, $team1, $team2]);
        if ((int)$teamReuse->fetchColumn() > 0) throw new Exception("Uno de los equipos ya está asignado a otra llave en esta fase");
    }
    ...
    error_log('Error copa_update_match: ' . $e->getMessage());
