...
function countTitulares(PDO $db, int $equipo_id, ?int $excludeId=null): int {
    $sql = "SELECT COUNT(*) FROM jugadores_segunda WHERE equipo_id=? AND es_titular=1";
    $params = [$equipo_id];
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $eq = $conn->prepare("SELECT * FROM equipos_segunda WHERE id=?");
    ...
    $st = $conn->prepare("
        ...
        LEFT JOIN estadisticas_jugadores_segunda e ON e.jugador_id=j.id AND e.temporada='2025-2026'
        WHERE j.equipo_id=?
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $conn->prepare("DELETE FROM jugadores_segunda WHERE id=?")->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($num !== null) {
            $chk = $conn->prepare("SELECT COUNT(*) FROM jugadores_segunda WHERE equipo_id=? AND numero_camiseta=?");
        ...
        if ($es_titular) {
            $cnt = $conn->prepare("SELECT COUNT(*) FROM jugadores_segunda WHERE equipo_id=? AND es_titular=1");
        ...
        $conn->prepare("INSERT INTO jugadores_segunda (equipo_id,nombre,posicion,numero_camiseta,edad,nacionalidad,foto,es_titular) VALUES (?,?,?,?,?,?,?,?)")
            ->execute([$equipo_id,$nombre,$posicion,$num,$edad,$nac,$foto,$es_titular]);
        $jugador_id = $conn->lastInsertId();

        $conn->prepare("INSERT IGNORE INTO estadisticas_jugadores_segunda (jugador_id,temporada) VALUES (?,'2025-2026')")
            ->execute([$jugador_id]);
    ...
    if ($action === 'update') {
        ...
        if ($num !== null) {
            $chk = $conn->prepare("SELECT COUNT(*) FROM jugadores_segunda WHERE equipo_id=? AND numero_camiseta=? AND id!=?");
        ...
        if ($es_titular) {
            $cnt = $conn->prepare("SELECT COUNT(*) FROM jugadores_segunda WHERE equipo_id=? AND es_titular=1 AND id!=?");
        ...
        $conn->prepare("UPDATE jugadores_segunda SET nombre=?,posicion=?,numero_camiseta=?,edad=?,nacionalidad=?,foto=?,es_titular=? WHERE id=?")
    ...
    if ($action === 'toggle_bulk') {
        ...
        $cur = $conn->prepare("SELECT id, es_titular FROM jugadores_segunda WHERE equipo_id=?");
        ...
        $stmt = $conn->prepare("UPDATE jugadores_segunda SET es_titular=? WHERE id=? AND equipo_id=?");
        foreach ($changes as $ch) {
    ...
    if ($action === 'update_stats') {
        ...
        $conn->prepare("
            INSERT INTO estadisticas_jugadores_segunda
    ...
    if ($action === 'save_formation') {
        ...
        $conn->prepare("UPDATE equipos_segunda SET formacion=? WHERE id=?")->execute([$formacion, $equipo_id]);
        $conn->prepare("UPDATE jugadores_segunda SET es_titular=0, posicion_x=NULL, posicion_y=NULL WHERE equipo_id=?")->execute([$equipo_id]);

        if (!empty($titulares)) {
            $st = $conn->prepare("UPDATE jugadores_segunda SET es_titular=1, posicion_x=?, posicion_y=? WHERE id=? AND equipo_id=?");
