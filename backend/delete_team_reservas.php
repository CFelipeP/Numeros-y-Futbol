...
$stmt = $conn->prepare("SELECT logo FROM equipos_reservas WHERE id = ?");
...
$stmt = $conn->prepare("DELETE FROM tabla_posiciones_reservas WHERE equipo_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->close();

$stmt = $conn->prepare("DELETE FROM equipos_reservas WHERE id = ?");
