...
if ((string)$g1 === '-1') {
    ...
    if ($m && $m['estado'] === 'Finalizado' && $m['goles_local'] !== null) {
        ...
        $stmt = $conn->prepare("UPDATE tabla_posiciones SET partidos_jugados = GREATEST(partidos_jugados-1,0) WHERE equipo_id IN (?, ?)");
        $stmt->bind_param("ii", $l, $v); $stmt->execute(); $stmt->close();

        $stmt = $conn->prepare("UPDATE tabla_posiciones SET goles_favor = GREATEST(goles_favor-?,0), goles_contra = GREATEST(goles_contra-?,0) WHERE equipo_id=?");
        ...
        if ($gl > $gv) {
            $stmt = $conn->prepare("UPDATE tabla_posiciones SET ganados=GREATEST(ganados-1,0), puntos=GREATEST(puntos-3,0) WHERE equipo_id=?");
            $stmt->bind_param("i", $l); $stmt->execute(); $stmt->close();
            $stmt = $conn->prepare("UPDATE tabla_posiciones SET perdidos=GREATEST(perdidos-1,0) WHERE equipo_id=?");
            ...
            $stmt = $conn->prepare("UPDATE tabla_posiciones SET empatados=GREATEST(empatados-1,0), puntos=GREATEST(puntos-1,0) WHERE equipo_id IN (?, ?)");
            $stmt->bind_param("ii", $l, $v); $stmt->execute(); $stmt->close();
        }
    }

    $stmt = $conn->prepare("UPDATE partidos SET goles_local=NULL, goles_visitante=NULL, estado='Pendiente' WHERE id=?");
...
$stmt = $conn->prepare("UPDATE tabla_posiciones SET goles_favor = goles_favor + ?, goles_contra = goles_contra + ? WHERE equipo_id=?");
...
if ($g1 > $g2) {
    $stmt = $conn->prepare("UPDATE tabla_posiciones SET ganados=ganados+1, puntos=puntos+3 WHERE equipo_id=?");
    $stmt->bind_param("i", $l); $stmt->execute(); $stmt->close();
    $stmt = $conn->prepare("UPDATE tabla_posiciones SET perdidos=perdidos+1 WHERE equipo_id=?");
    $stmt->bind_param("i", $v); $stmt->execute(); $stmt->close();
} elseif ($g1 < $g2) {
    $stmt = $conn->prepare("UPDATE tabla_posiciones SET ganados=ganados+1, puntos=puntos+3 WHERE equipo_id=?");
    $stmt->bind_param("i", $v); $stmt->execute(); $stmt->close();
