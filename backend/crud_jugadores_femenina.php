...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_formation') {
        ...
        $conn->prepare("UPDATE jugadores_femenina SET es_titular=0, posicion_x=NULL, posicion_y=NULL, pos_x=NULL, pos_y=NULL WHERE equipo_id=?")->execute([$equipo_id]);

        if (!empty($titulares)) {
            $st = $conn->prepare("UPDATE jugadores_femenina SET es_titular=1, pos_x=?, pos_y=? WHERE id=? AND equipo_id=?");
