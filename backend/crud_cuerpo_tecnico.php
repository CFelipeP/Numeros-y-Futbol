...
try {
    if ($method === 'GET') {
        $stmt = $conn->prepare("SELECT * FROM cuerpo_tecnico_seleccion ORDER BY id ASC");
        $stmt->execute();
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            $stmt = $conn->prepare("
                INSERT INTO cuerpo_tecnico_seleccion (nombre, rol, foto, nacionalidad)
                VALUES (?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['nombre'],
        ...
        if ($action === 'update') {
            ...
            if (empty($data['nombre'] ?? '')) { echo json_enc(["success" => false, "error" => "El nombre es requerido"]); exit; }
            $stmt = $conn->prepare("
                UPDATE cuerpo_tecnico_seleccion SET
        ...
        if ($action === 'delete') {
            $id = isset($data['id']) ? (int)$data['id'] : 0;
            if (!$id) { echo json_enc(["success" => false, "error" => "ID requerido"]); exit; }
            $stmt = $conn->prepare("DELETE FROM cuerpo_tecnico_seleccion WHERE id = ?");
            $stmt->execute([$id]);
            echo json_enc(["success" => true]);
            exit;
        }
