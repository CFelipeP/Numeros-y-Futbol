...
function normalizarPosicion($pos, $validas, $mapeo) {
    if (!$pos) return 'centrodelantero';
    $pos = strtolower(trim($pos));
    if (in_array($pos, $validas)) return $pos;
    return $mapeo[$pos] ?? 'centrodelantero';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $st = $conn->prepare("
        SELECT j.*,
               ...
               e.temporada
        FROM jugadores j
        LEFT JOIN estadisticas_jugadores e
               ON e.jugador_id = j.id AND e.temporada = '2025-2026'
        WHERE j.equipo_id = ?
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $st = $conn->prepare("DELETE FROM jugadores WHERE id=?");
    $st->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $posicion   = normalizarPosicion($data['posicion'] ?? '', $POSICIONES_VALIDAS, $POSICION_MAPEO);
        ...
        if ($es_titular) {
            $c = $conn->prepare("SELECT COUNT(*) FROM jugadores WHERE equipo_id=? AND es_titular=1");
            $c->execute([$equipo_id]);
            if ($c->fetchColumn() >= 11) {
        ...
        $st = $conn->prepare("INSERT INTO jugadores (equipo_id,nombre,posicion,numero_camiseta,edad,nacionalidad,foto,es_titular) VALUES (?,?,?,?,?,?,?,?)");
        $st->execute([$equipo_id,$nombre,$posicion,$num,$edad,$nac,$foto,$es_titular]);
        $jugador_id = $conn->lastInsertId();

        // Crear fila de estadísticas vacía
        $conn->prepare("INSERT IGNORE INTO estadisticas_jugadores (jugador_id,temporada) VALUES (?,'2025-2026')")->execute([$jugador_id]);
    ...
    if ($action === 'update') {
        ...
        // Verificar límite 11 titulares (excluyendo el jugador actual)
        if ($es_titular) {
            $c = $conn->prepare("SELECT COUNT(*) FROM jugadores WHERE equipo_id=? AND es_titular=1 AND id!=?");
            $c->execute([$equipo_id, $id]);
            if ($c->fetchColumn() >= 11) {
                echo json_encode(['success'=>false,'error'=>'Ya hay 11 titulares. Baja uno primero.']);
                exit();
            }
        ...
        $st = $conn->prepare("UPDATE jugadores SET nombre=?,posicion=?,numero_camiseta=?,edad=?,nacionalidad=?,foto=?,es_titular=? WHERE id=?");
        $st->execute([$nombre,$posicion,$num,$edad,$nac,$foto,$es_titular,$id]);

        echo json_encode(['success'=>true]);
        exit();
    }

    // ── TOGGLE BULK (nuevo: cambiar varios titulares de golpe) ───────────────
    // action=toggle_bulk: { changes: [{id, es_titular}, ...], equipo_id }
    if ($action === 'toggle_bulk') {
        ...
        // Calcular cuántos titulares quedarán
        // 1. Estado actual de todos los jugadores del equipo
        ...
        foreach ($cur->fetchAll(PDO::FETCH_ASSOC) as $row) {
            $estado[$row['id']] = (bool)$row['es_titular'];
        }
        // 2. Aplicar cambios en memoria
        foreach ($changes as $ch) {
            $estado[intval($ch['id'])] = (bool)$ch['es_titular'];
        }
        // 3. Contar titulares resultantes
        ...
        foreach ($changes as $ch) {
            $stmt->execute([(int)(bool)$ch['es_titular'], intval($ch['id']), $equipo_id]);
        }
    ...
    if ($action === 'update_stats') {
        ...
        $temporada          = $data['temporada'] ?? '2025-2026';
        $pj                 = intval($data['pj'] ?? 0);
        $goles              = intval($data['goles'] ?? 0);
        $asistencias        = intval($data['asistencias'] ?? 0);
        $goles_cabeza       = intval($data['goles_cabeza'] ?? 0);
        $goles_tiro_libre   = intval($data['goles_tiro_libre'] ?? 0);
        $goles_penal        = intval($data['goles_penal'] ?? 0);
        $tarjetas_amarillas = intval($data['tarjetas_amarillas'] ?? 0);
        $tarjetas_rojas     = intval($data['tarjetas_rojas'] ?? 0);
        $minutos_jugados    = intval($data['minutos_jugados'] ?? 0);
        $goles_recibidos    = intval($data['goles_recibidos'] ?? 0);
        $vaya_invicta       = intval($data['vaya_invicta'] ?? 0);
        ...
        ");
        $st->execute([$jugador_id,$temporada,$pj,$goles,$asistencias,$goles_cabeza,
                      $goles_tiro_libre,$goles_penal,$tarjetas_amarillas,$tarjetas_rojas,
                      $minutos_jugados,$goles_recibidos,$vaya_invicta]);
    ...
    if ($action === 'save_formation') {
        ...
        // Primero poner todos a suplente
