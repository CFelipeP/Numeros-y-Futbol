...
try {
    if ($method === 'GET') {
        $stmt = $conn->prepare("SELECT * FROM partidos_seleccion_sub20 ORDER BY fecha DESC, id DESC");
        $stmt->execute();
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            $stmt = $conn->prepare("
                INSERT INTO partidos_seleccion_sub20 (rival_nombre, rival_logo, goles_favor, goles_contra, fecha, hora, estado, competicion, lugar)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['rival_nombre'],
        ...
        if ($action === 'update') {
            ...
            if (empty($data['rival_nombre'] ?? '')) { echo json_enc(["success" => false, "error" => "El nombre del rival es requerido"]); exit; }
            $stmt = $conn->prepare("
                UPDATE partidos_seleccion_sub20 SET
        ...
        if ($action === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM partidos_seleccion_sub20 WHERE id = ?");
            $stmt->execute([$id]);
            echo json_enc(["success" => true]);
            exit;
        }
