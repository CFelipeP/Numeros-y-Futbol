...
if (isset($_FILES['logo']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
    ...
    $nuevoNombre = 'reservas_' . $id . '_' . time() . '.' . $extension;
...
if ($logoPath) {
    $stmt = $conn->prepare("UPDATE equipos_reservas SET nombre = ?, ciudad = ?, estadio = ?, logo = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $nombre, $ciudad, $estadio, $logoPath, $id);
} else {
    $stmt = $conn->prepare("UPDATE equipos_reservas SET nombre = ?, ciudad = ?, estadio = ? WHERE id = ?");
