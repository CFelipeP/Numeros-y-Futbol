...
if ($jornada) {
    $stmt = $conn->prepare("SELECT $selectFields FROM partidos_burgerking p$joinEquipos WHERE p.jornada = ? AND p.estado = 'Finalizado' ORDER BY p.fecha ASC");
    $stmt->bind_param("i", $jornada); $stmt->execute(); $res = $stmt->get_result();
    while ($row = $res->fetch_assoc()) $recent[] = $row;
} else {
    $res = $conn->query("SELECT $selectFields FROM partidos_burgerking p$joinEquipos WHERE p.estado = 'Finalizado' ORDER BY p.fecha DESC, p.id DESC LIMIT 10");
