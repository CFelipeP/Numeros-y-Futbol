...
try {
    if ($method === 'GET') {
        $stmt = $conn->prepare("SELECT * FROM jugadores_seleccion_femenina ORDER BY numero_camiseta ASC");
        $stmt->execute();
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            $stmt = $conn->prepare("
                INSERT INTO jugadores_seleccion_femenina (nombre, posicion, numero_camiseta, foto, edad, nacionalidad, club_origen, partidos_jugados, goles, asistencias, atajadas)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['nombre'],
        ...
        if ($action === 'update') {
            ...
            if (empty($data['nombre'] ?? '')) { echo json_enc(["success" => false, "error" => "El nombre es requerido"]); exit; }
            $stmt = $conn->prepare("
                UPDATE jugadores_seleccion_femenina SET
        ...
        if ($action === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM jugadores_seleccion_femenina WHERE id = ?");
            $stmt->execute([$id]);
            echo json_enc(["success" => true]);
            exit;
        }
