...
if (!empty($_FILES['logo']['name'])) {
    ...
    $stmt2 = $conn->prepare("UPDATE equipos_segunda SET nombre=?, ciudad=?, estadio=?, logo=? WHERE id=?");
    $stmt2->execute([$nombre, $ciudad, $estadio, $ruta, $id]);
} else {
    $stmt2 = $conn->prepare("UPDATE equipos_segunda SET nombre=?, ciudad=?, estadio=? WHERE id=?");
