...
if ($jornada) {
    $stmt = $conn->prepare("SELECT p.id, p.fecha, p.goles_local, p.goles_visitante, p.estado AS status, p.jornada, p.featured, e1.nombre AS home_name, e1.logo AS home_logo, e2.nombre AS away_name, e2.logo AS away_logo FROM partidos_femenina p LEFT JOIN equipos_primera_femenina e1 ON p.equipo_local = e1.id LEFT JOIN equipos_primera_femenina e2 ON p.equipo_visitante = e2.id WHERE p.jornada = ? AND p.estado = 'Finalizado' ORDER BY p.fecha DESC, p.id DESC LIMIT 6");
