...
try {
    if ($method === 'GET') {
        $stmt = $conn->prepare("SELECT * FROM cuerpo_tecnico_seleccion_sub20 ORDER BY id ASC");
        $stmt->execute();
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'create') {
            ...
            $stmt = $conn->prepare("
                INSERT INTO cuerpo_tecnico_seleccion_sub20 (nombre, rol, foto, nacionalidad)
                VALUES (?, ?, ?, ?)
            ");
            $stmt->execute([
                $data['nombre'],
        ...
        if ($action === 'update') {
            ...
            if (empty($data['nombre'] ?? '')) { echo json_enc(["success" => false, "error" => "El nombre es requerido"]); exit; }
            $stmt = $conn->prepare("
                UPDATE cuerpo_tecnico_seleccion_sub20 SET
        ...
        if ($action === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM cuerpo_tecnico_seleccion_sub20 WHERE id = ?");
            $stmt->execute([$id]);
            echo json_enc(["success" => true]);
            exit;
        }
